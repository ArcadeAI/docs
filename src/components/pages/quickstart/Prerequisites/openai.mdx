...
Create a new file called `example_openai.py` and enter the following code:
...
Create a new file called `example_openai.js` and enter the following code:
...
You can now call tools with the client. Here's an example of calling the `Math.Sqrt` tool using OpenAI's chat completion:
