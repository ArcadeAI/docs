...
describe("toolkitDataToSearchMarkdown", () => {
  ...
  it("includes enum values for parameters", () => {
    ...
    expect(markdown).toContain("- Allowed values: `open`, `closed`, `all`");
  });
  ...
  it("includes quick reference section with links", () => {
    ...
    expect(markdown).toContain("- [`Github.CreateIssue`](#githubcreateissue)");
    expect(markdown).toContain("- [`Github.ListIssues`](#githublistissues)");
  });
