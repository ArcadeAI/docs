 ...
 *
 * Comparison is structural (by JSON path), not raw-byte. `generatedAt` is
 * volatile, and `curationSourceHash` records which curation produced a file
 * rather than anything a reader sees. `summary`'s key *position* can also vary
 * across generator versions without changing content. Any rendered-content
 * difference exits non-zero and names the file and JSON path.
 *
 * Curation is compiled from the same authoritative Markdown/MDX source used by
 * generation. Enrichment defaults to the committed toolkit JSON until it has
 * its own extracted source.
 *
 ...
 *     [--enrichment data/toolkits] [--curation curation] \
...
/** Root fields that are not part of rendered toolkit parity. */
const IGNORED_ROOT_KEYS = ["generatedAt", "curationSourceHash"];
 ...
 * emitted one, ignoring non-rendered root fields. Returns null when equal.
 */
export const firstDifference = (
  ...
  const strip = (value: Record<string, unknown>): Record<string, unknown> => {
    const copy = { ...value };
    for (const key of IGNORED_ROOT_KEYS) {
      delete copy[key];
    }
    return copy;
  };
...
const DEFAULT_CURATION_DIR = fileURLToPath(
  new URL("../curation", import.meta.url)
);
  ...
  if (!(values.snapshot && values.reference)) {
    throw new Error(
      "Usage: verify-toolkit-join.ts --snapshot <file> --reference <dir> " +
        "[--enrichment <dir>] [--curation <mdx-dir>] [--metadata <file>]"
    );
  }
  return {
    ...
    curation: values.curation ?? DEFAULT_CURATION_DIR,
...
const loadCatalogTools = async (
  ...
  if (!Array.isArray(raw.items)) {
    throw new Error("Invalid catalog snapshot: items must be an array");
  }
  const recordedTotal = raw.total_count ?? raw.totalCount;
  if (typeof recordedTotal !== "number" || !Number.isInteger(recordedTotal)) {
    throw new Error(
      "Invalid catalog snapshot: total_count/totalCount must be an integer"
    );
  }
  if (raw.items.length !== recordedTotal) {
    throw new Error(
      `Truncated catalog snapshot: collected ${raw.items.length} tools but total_count is ${recordedTotal}`
    );
  }
  const { items } = parseToolMetadataResponse({
    items: raw.items,
    total_count: recordedTotal,
  });
...
async function main(): Promise<void> {
  ...
  const curationSource = createMarkdownCurationSource(options.curation);
  ...
  for (const file of files) {
    ...
    const curation = await curationSource.getCustomSections(toolkitId);
