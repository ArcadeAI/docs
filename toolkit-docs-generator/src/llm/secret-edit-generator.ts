...
const buildCoveragePrompt = (input: SecretCoverageEditInput): string => {
  ...
  return [
    ...
    "- Each missing secret should get at most one short, factual line describing what it is and, when possible, how a developer obtains it (e.g. provider Dashboard, region URL, API key). If you do not know, keep it to the name and a one-line purpose — do not invent provisioning steps.",
    ...
    linkInstruction,
    "- Keep the output compact: no filler, no marketing copy. The summary should remain easy to scan.",
    "",
