...
The Canva  enables tools and  to call [Canva APIs](https://developers.canva.com/docs/rest-api/)  on behalf of a  using OAuth 2.0 authentication.

Want to quickly get started with Canva in your  or AI app? The pre-built [Arcade Canva MCP Server](/resources/integrations/development/canva.md) is what you want!
...
-   The [Arcade Canva MCP Server](/resources/integrations/development/canva.md)
    , which provides pre-built  for interacting with Canva
-   Your [app code](#using-canva-auth-in-app-code)
     that needs to call the Canva API
-   Or, your [custom tools](#using-canva-auth-in-custom-tools)
     that need to call the Canva API

### Required scopes for the Canva MCP Server

If you’re using the [Arcade Canva MCP Server](/resources/integrations/development/canva.md), you’ll need to configure these scopes based on which  you plan to use:
...
The `projects:read` scope is **only available in private Canva OAuth apps**. If you need to access team  and files, you must create a private OAuth app through your Canva organization.
...
In a production environment, you will most likely want to use your own Canva app credentials. This way, your  will see your application’s name requesting permission.
...
Navigate to the [Canva Developer Portal](https://www.canva.com/developers/)  and sign in with your existing Canva credentials or create a new .
...
**Note on `projects:read` scope:** If you need access to the `projects:read` scope for team  and files navigation, you must create a **private Canva OAuth app**. This scope is not available in public OAuth apps. Learn more in the [Canva OAuth Scopes documentation](https://developers.canva.com/docs/rest-api/scopes/) .

When you use tools that require Canva auth using your Arcade  credentials, Arcade will automatically use this Canva OAuth provider. If you have multiple Canva providers, see [using multiple auth providers of the same type](/references/auth-providers.md#using-multiple-providers-of-the-same-type) for more information.

## Using Canva auth in app code

Use the Canva  in your own  and AI apps to get a  token for the Canva API. See [authorizing agents with Arcade](/get-started/about-arcade.md) to understand how this works.

Use `client.auth.start()` to get a  token for the Canva API:
...
You can use the pre-built [Arcade Canva MCP Server](/resources/integrations/development/canva.md) to quickly build  and AI apps that interact with Canva.

If the pre-built tools in the Canva  Server don’t meet your needs, you can author your own [custom tools](/guides/create-tools/tool-basics/build-mcp-server.md) that interact with the Canva API.

Use the `Canva()` auth class to specify that a  requires authorization with Canva. The `context.authorization.token` field will be automatically populated with the ’s Canva token:
