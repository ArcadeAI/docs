...
// Pattern to identify toolkit-generated integration files
// These are generated by toolkit-docs-generator, not from MDX sources
const TOOLKIT_INTEGRATION_PATTERN =
  /\/resources\/integrations\/(?:customer-support|databases|development|entertainment|payments|productivity|sales|search|social-communication)\//;

/**
 * Removes orphaned markdown files that no longer have corresponding MDX sources
 */
async function cleanupOrphanedMarkdown(
  ...
  // Find all existing markdown files in the output directory
  const existingFiles = glob.sync("**/*.md", {
    cwd: OUTPUT_DIR,
    absolute: true,
  });
  ...
  for (const filePath of existingFiles) {
    ...
    // Skip toolkit-generated integration files (they have a different source)
    if (TOOLKIT_INTEGRATION_PATTERN.test(filePath)) {
      continue;
    }
    ...
    if (!expectedPaths.has(normalizedPath)) {
      try {
        ...
        console.error(pc.yellow(`  ⚠ Could not remove ${filePath}: ${error}`));
      }
    }
  }
  ...
  for (const dir of dirs) {
    ...
    try {
      ...
      // Directory may not exist or have permissions issues
    }
  }
...
async function main() {
  ...
  try {
    ...
    // Build set of expected output paths for cleanup
    const expectedPaths = new Set(
      pages.map((p) => path.normalize(p.outputPath))
    );

    // Remove orphaned markdown files
    console.log(pc.blue("\n🧹 Cleaning up orphaned markdown files...\n"));
    const { removed, paths: removedPaths } =
      await cleanupOrphanedMarkdown(expectedPaths);
    ...
    if (removed > 0) {
      console.log(pc.yellow(`  🗑️  Removed orphaned files: ${removed}`));
      for (const removedPath of removedPaths) {
        const relativePath = path.relative(OUTPUT_DIR, removedPath);
        console.log(pc.gray(`     - ${relativePath}`));
      }
    }
