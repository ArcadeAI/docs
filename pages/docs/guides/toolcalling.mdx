...
To do this, you'll write code that uses Arcade AI to call both public and authorized tools.
...
```python
auth_status = client.auth.status(auth_response)
...
You only need to pass the `user_id`:
...
When you call a tool with `client.tool.run()`, Arcade AI:

1. Checks for authorization.
2. Routes the request to the tool's provider.
3. Returns the tool's response.

With `client.auth.authorize()`, you can also authorize tools for later use.

These APIs give you programmatic control over tool calling. If you just want to integrate tools into an LLM workflow, see [Call tools with LLMs](/docs/guides/llmtoolcalling).

### Next steps

Once you've mastered tool calling, see how you can [build your own tools](/docs/guides/customtools) to integrate any custom functionality or API to your AI workflows.
