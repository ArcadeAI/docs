...
- `routers` - Defines routers per model type (e.g., `routers.language`, `routers.speech`). See [router types](/essentials/routers) for more details.
...
A simple setup includes a single router and two models. This example shows a failover router using OpenAI and Azure OpenAI. For OpenAI, Cohere, and OctoML, only an API key is needed. For Azure OpenAI, specify a model and base_url.

- `language` - Language API endpoint supporting `/chat` from model providers
- `id` - Unique router ID
- `strategy` - Router type: round-robin, weighted-round-robin, least-latency, priority
...
See [advanced configuration](/essentials/configuration#advanced-configuration) for more details.
