...
How you configure the Spotify auth provider depends on whether you use the Arcade [Cloud Engine](/home/cloud-engine) or a [self-hosted Engine](/guides/installation).
...
Use the Spotify auth provider in your own agents and AI apps to get a user token for the Spotify API. See [authorizing agents with Arcade](/guides/agentauth) to understand how this works.

Use `client.auth.authorize()` to get a user token for the Spotify API:

```python file=<rootDir>/examples/code/integrations/spotify/custom_auth.py {9-13}

```

## Using Spotify auth in custom tools

The Arcade [LLM API](/guides/llmtoolcalling) is a convenient way to call LLMs and automatically invoke tools. You can author your own [custom tools](/guides/customtools) that interact with the Spotify API.
...
```python file=<rootDir>/examples/code/integrations/spotify/custom_tool.py {7,10-14,20}
