...
- Your [app code](#using-discord-auth-in-your-app-code) that needs to call Discord APIs
- Or, your [custom tools](#using-discord-auth-in-customtools) that need to call Discord APIs

## Configuring Discord auth

How you configure the Discord auth provider depends on whether you use the Arcade Cloud Engine or a [self-hosted Engine](/home/install/overview).

With the Arcade Cloud Engine, you can start building and testing Discord auth without any configuration. Your users will see `Arcade AI demo` as the name of the application that's requesting permission.

When you are ready to go to production, you'll want to configure the Discord auth provider with your own Discord app credentials, so users see your app name when they authorize access.
...
The Arcade [LLM API](/home/use-tools/call-tools-with-llms) is a convenient way to call LLMs and automatically invoke tools. You can author your own [custom tools](/home/build-tools/create-a-toolkit) that interact with the Discord API.
