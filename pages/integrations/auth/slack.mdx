...
<Tip>
  ...
  [Arcade Slack toolkit](/integrations/toolkits/Slack/slack) is what you want!
</Tip>
...
- The [Arcade Slack toolkit](/integrations/toolkits/Slack/slack), which provides pre-built tools for interacting with Slack
- Your [app code](#using-slack-auth-in-your-app-code) that needs to call the Slack API
- Or, your [custom tools](#using-slack-auth-in-customtools) that need to call the Slack API

## Configuring Slack auth

How you configure the Slack auth provider depends on whether you use the Arcade [Cloud Engine](/home/cloud-engine) or a [self-hosted Engine](/guides/installation).
...
Use the Slack auth provider in your own agents and AI apps to get a user token for the Slack API. See [authorizing agents with Arcade](/guides/agentauth) to understand how this works.

Use `client.auth.authorize()` to get a user token for the Slack API:

```python file=<rootDir>/examples/code/integrations/slack/custom_auth.py {9-18}

```

## Using Slack auth in custom tools

The Arcade [LLM API](/guides/llmtoolcalling) is a convenient way to call LLMs and automatically invoke tools. You can use the pre-built [Arcade Slack toolkit](/integrations/toolkits/Slack/slack) to quickly build agents and AI apps that interact with Slack.

If the pre-built tools in the Slack toolkit don't meet your needs, you can author your own [custom tools](/guides/customtools) that interact with the Slack API.
...
```python file=<rootDir>/examples/code/integrations/slack/custom_tool.py {8,11-20,31}
