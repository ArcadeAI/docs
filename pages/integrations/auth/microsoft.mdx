...
- Your [app code](#using-microsoft-auth-in-your-app-code) that needs to call Microsoft Graph APIs
- Or, your [custom tools](#using-microsoft-auth-in-customtools) that need to call Microsoft Graph APIs

## Configuring Microsoft auth

How you configure the Microsoft auth provider depends on whether you use the Arcade [Cloud Engine](/home/cloud-engine) or a [self-hosted Engine](/guides/installation).
...
Use the Microsoft auth provider in your own agents and AI apps to get a user token for Microsoft Graph APIs. See [authorizing agents with Arcade](/guides/agentauth) to understand how this works.

Use `client.auth.authorize()` to get a user token for Microsoft Graph APIs:

```python file=<rootDir>/examples/code/integrations/microsoft/custom_auth.py {9-13}

```

## Using Microsoft auth in custom tools

The Arcade [LLM API](/guides/llmtoolcalling) is a convenient way to call LLMs and automatically invoke tools. You can author your own [custom tools](/guides/customtools) that interact with Microsoft Graph APIs.
...
```python file=<rootDir>/examples/code/integrations/microsoft/custom_tool.py {7,10-14,21}
