...
How you configure the LinkedIn auth provider depends on whether you use the Arcade [Cloud Engine](/home/cloud-engine) or a [self-hosted Engine](/guides/installation).
...
Use the LinkedIn auth provider in your own agents and AI apps to get a user token for LinkedIn APIs. See [authorizing agents with Arcade](/guides/agentauth) to understand how this works.

Use `client.auth.authorize()` to get a user token for LinkedIn APIs:
...
The Arcade [LLM API](/guides/llmtoolcalling) is a convenient way to call LLMs and automatically invoke tools. You can author your own [custom tools](/guides/customtools) that interact with LinkedIn APIs.
