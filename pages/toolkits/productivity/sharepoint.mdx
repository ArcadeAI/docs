...
<TableOfContents
  ...
  data={
    [
      ...
      ["Sharepoint.DownloadFileFromDrive", "Download a file from a drive in a SharePoint site."],
    ]
  }
/>
...
## Sharepoint.DownloadFileFromDrive
  ...
  tabs={[
    {
      label: "Call the Tool Directly",
      content: {
        Python: ["/examples/integrations/toolkits/sharepoint/download_file_from_drive_example_call_tool.py"],
        JavaScript: ["/examples/integrations/toolkits/sharepoint/download_file_from_drive_example_call_tool.js"],
      },
    },
  ]}
/>

Download a file from a drive in a SharePoint site.

**Parameters**

- **drive_id** (`string`, required) The ID of the drive to download the item from.
- **file_id** (`string`, required) The ID of the file to download.
- **encoding** (`string`, optional) The encoding to use for the downloaded file. Defaults to 'utf-8'. This is only used in case the file is of text type. When the file is binary, the content is returned as base64 encoded string.
