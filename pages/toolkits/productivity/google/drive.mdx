...
<<<<<<<< HEAD:pages/toolkits/productivity/google/drive.mdx
...
========
- **`corpora`** _(enum ([Corpora](/integrations/toolkits/productivity/google/reference#corpora)), optional)_ The source of files to list. Defaults to `Corpora.USER`.
- **`title_keywords`** _(list[str], optional)_ Keywords or phrases that must be in the document title. Defaults to `None`.
- **`order_by`** _(enum ([OrderBy](/integrations/toolkits/productivity/google/reference#orderby)), optional)_ Sort order. Defaults to `OrderBy.MODIFIED_TIME_DESC`.
>>>>>>>> main:pages/integrations/toolkits/productivity/google/drive.mdx
...
The Arcade AI Drive toolkit uses the [Google auth provider](/home/auth-providers/google) to connect to users' Google accounts.
