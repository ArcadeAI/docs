...
<Warning type="warning">
  When using [model calling](/home/use-tools/call-tools-with-models) with the `Google.FindTimeSlotsWhenEveryoneIsFree` tool, always ground the LLM with the current date and day of the week by appending it to the prompt. See examples above for "Execute the Tool with OpenAI".
</Warning>
