...
- Create and update issues with specific details like project, priority, and assignee.
...
- Retrieve information about projects, users, and issue types.
...
- **labels** (`array[string]`, required) The labels to add to the issue
...
- **project** (`string`, optional) The ID, key or name of the project to associate the issue with. If a name is provided, the tool will try to find a unique exact match among the available projects. Defaults to None (no project). Must provide at least one of `project` or `parent_issue` arguments.
...
- **labels** (`array[string]`, optional) The labels of the issue. Defaults to None (no labels).
...
- **issue** (`string`, required) The ID, key or name of the issue to retrieve priorities for.
