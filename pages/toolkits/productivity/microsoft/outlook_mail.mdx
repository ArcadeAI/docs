...
- **`cc_recipients`** (list[str] | None, optional): The email addresses that will be the CC recipients of the email.
- **`bcc_recipients`** (list[str] | None, optional): The email addresses that will be the BCC recipients of the email.
  ...
  tabs={[
    {
      label: "Call the tool with user authorization",
      content: {
        Python: ["/examples/integrations/toolkits/microsoft/outlook_mail/reply_all_to_email_example_call_tool.py"],
        JavaScript: ["/examples/integrations/toolkits/microsoft/outlook_mail/reply_all_to_email_example_call_tool.js"]
      }
    },
    {
      label: "Execute the tool with OpenAI",
      content: {
        Python: ["/examples/integrations/toolkits/microsoft/outlook_mail/reply_all_to_email_example_llm_oai.py"],
        JavaScript: ["/examples/integrations/toolkits/microsoft/outlook_mail/reply_all_to_email_example_llm_oai.js"]
      }
    }
  ]}
/>
...
- **`pagination_token`** (str | None, optional): The pagination token to continue a previous request
...
- **`well_known_folder_name`** (WellKnownFolderNames | None, optional): The name of the folder to list emails from. Defaults to None.
- **`folder_id`** (str | None, optional): The ID of the folder to list emails from if the folder is not a well-known folder. Defaults to None.
- **`limit`** (int, optional): The number of messages to return. Max is 100. Defaults to 5.
- **`pagination_token`** (str | None, optional): The pagination token to continue a previous request
