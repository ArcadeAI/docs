...
```python file=<rootDir>/examples/code/home/use-tools/call-tools-with-llms/single_llm_call.py#L14-L26 {12}
...
Additionally, these options from OpenAI's `tool_choice` parameter are supported, but not commonly used:
...
- **Tool Execution**: The Engine requests the Actor hosting the Calendar toolkit to retrieve events for tomorrow.
