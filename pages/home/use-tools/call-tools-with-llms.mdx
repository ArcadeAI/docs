...
Use the `OpenAI` client, but change the base URL and API key to point to Arcade AI:

```python file=<rootDir>/examples/code/home/use-tools/call-tools-with-llms/github_stars.py#L1-L8 {6-7}
...
```python file=<rootDir>/examples/code/home/use-tools/call-tools-with-llms/github_stars.py#L10-L11
...
```python file=<rootDir>/examples/code/home/use-tools/call-tools-with-llms/github_stars.py#L13-
...
```bash
python github_stars.py # or example.py
```

Ask the AI to:

```
star the arcade-ai/arcade-ai repo
```
...
Add more tools to the `tools` array to give the AI model more capabilities.
...
For apps and agents that aren't directly generating chat completions, you can [authorize and call tools programmatically](/home/use-tools/call-tools-in-code). This is useful when you need more control over the tool calling process.

When you're ready to build your own tools, read about [how to create tools](/home/build-tools).
