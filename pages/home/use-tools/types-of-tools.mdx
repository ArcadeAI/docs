...
description: "Learn about LLM-native and API Proxy tools"
---
...
Since they require careful design and evaluation, LLM-native tools take time and effort to build. We understand that your Agent or chatbot project might need capabilities not yet covered by our LLM-native toolkits. For this reason, we also offer low-level API Proxy toolkits.

## API Proxy tools

To provide your Agent or chatbot with more freedom to interact with the upstream services, we offer API Proxy toolkits.

API Proxy tools are heavily influenced by the original API design. Each tool mirrors one HTTP endpoint.

Although we redesign the tool name and argument descriptions to make them more suitable for LLMs, API Proxy tools are still not optimized for LLM usage. Also, they are not subject to evaluation suites like LLM-native tools. For those reasons, we recommend thoroughly evaluating each API Proxy tool with your Agents or chatbots before using it in production.

When your Agent's needs are covered by an LLM-native tool, we recommend using it instead of an API Proxy. Use API Proxies as a complement. Carefully engineer your prompts to ensure your Agent can call them safely.
