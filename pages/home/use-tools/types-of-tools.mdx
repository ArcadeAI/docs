...
Arcade builds two types of tools within our toolkit ecosystem:

- Starter tools: Auto-generated tools that mirror HTTP endpoints
- Optimized tools: Hand-crafted tools designed for LLM usage

These represent the two categories of tools that Arcade develops and maintains. Additionally, our platform hosts Verified and Community toolkits created by our developer community, as well as Auth Provider integrations.

The distinction between Starter and Optimized tools is a matter of design philosophy and development approach. The distinction is merely a matter of how they are designed. Both types of tools can be used seamlessly in the same way. There is no difference in their interfaces, the way they are called, or how you interact with them through the Arcade [Dashboard](https://api.arcade.dev/dashboard/) or the Arcade [SDK clients](/home/arcade-clients).
...
Traditionally, the HTTP APIs offered by upstream services such as GitHub, Google, Slack, etc., were designed to be consumed by human software engineers. When we exposed such interfaces for LLMs to call as tools, they usually did not perform well.
...
Even the most powerful LLMs usually perform poorly when they need to reason such complex workflows on the fly, not to mention the increased cost and risk of hallucinations. This makes AI Agents and chatbots that rely on HTTP APIs unreliable.
...
Although we redesign the tool name and argument descriptions to make them more suitable for LLMs, Starter tools are not optimized for LLM usage. They are also not subject to evaluation suites like Optimized tools. For those reasons, we recommend thoroughly evaluating each Starter tool with your Agents or chatbots before use in production.

We recommend using Optimized tools whenever they meet your Agent's requirements. When you need additional functionality, use Starter tools as a complement. Just be sure to carefully engineer your prompts to ensure your Agent can call them safely.