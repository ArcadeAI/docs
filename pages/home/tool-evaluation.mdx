...
that these models can not only efficiently utilize the available tools (traditional testing) but also understand user
intents and predict the correct tools to use to fulfill those intents (evaluation).

The Arcade AI Evaluation Framework provides a structured approach to evaluate and validate the tool-calling capabilities
 of LLMs, ensuring they perform as expected.
...
- **Result**:
    - *Passed*: Score is above the fail threshold.
    - *Failed*: Score is below the fail threshold.
    - *Warned*: Score is between the warning and fail thresholds.
