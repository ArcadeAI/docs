...
<Tip>
  ...
  pre-built [Arcade Asana toolkit](/toolkits/productivity/asana) is what
  you want!
</Tip>
...
<Note>
  If you're adding more than one Asana provider and your app relies on the built-in toolkit, please ensure to set the **ID** of one your Asana providers to `asana`. This ensures the Arcade Engine will connect the built-in tools to that provider. [Read more](/home/faq#can-i-add-multiple-providers-of-the-same-type).
</Note>
...
When you use tools that require Asana auth using your Arcade account credentials, the Arcade Engine will automatically use this Asana OAuth provider.
...
<Note>
  As explained [above](#create-an-asana-app), the Asana granular permission scopes are in preview and not yet supported. The `"default"` scope should be used as the only scope in all tools.
</Note>
