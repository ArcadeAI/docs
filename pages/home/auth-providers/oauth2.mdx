...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L8-L25 {11-18}
...
`scope_delimiter` _(optional, defaults to the space character)_: The delimiter for scopes, if any.
...
- `parameters`: A map of parameter keys and values to include in the authorization request.

These placeholders are available in `parameters`:
...
<Tip>
  ...
  to be included in `parameters`. If PKCE is enabled, `code_challenge` and
  `code_challenge_method` are also added automatically.
</Tip>

For most providers, `oauth2.authorize_request.parameters` will look like:
...
Some providers support additional parameters in the authorization request. These can be added to `parameters` as well. For example:
...
- `parameters`: A map of parameter keys and values to include in the token request.
...
<Tip>
  ...
  to be included in `parameters`. If PKCE is enabled, the `code_verifier` parameter
  is also added to the request automatically.
</Tip>

For most providers, `oauth2.token_request.parameters` will look like:
...
- `parameters`: A map of parameter keys and values to include in the refresh token request.
...
For most providers, `oauth2.refresh_request.parameters` will look like:
...
In the `token_request` and `refresh_request` sections, you can optionally configure `response_map`. Configuring `response_map` is useful if your OAuth 2.0 server returns a JSON object with nested properties, or properties with non-standard names.
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L8-L71 {11-71}
