...
Follow Salesforce's guide to [creating your Connected App](https://help.salesforce.com/s/articleView?id=xcloud.connected_app_create.htm&type=5)

When creating your app, make sure to:

- Check the **Enable OAuth Settings** box
- Set the callback URL to: `https://cloud.arcade.dev/api/v1/oauth/callback`
- In the **Available OAuth Scopes**, add the **Manage User Data Via APIs (api)** scope
...
After your app is created, take note of the client ID and client secret to use in the next steps.
...
1. Under **My Domain Details**, check the **Current My Domain URL** field.
...
By default, Salesforce offers only one [OAuth scope](https://oauth.net/2/scope/) (`api`) that is applicable to using their CRM REST API. The developer is tasked with creating [OAuth custom scopes](https://help.salesforce.com/s/articleView?id=xcloud.remoteaccess_oauth_customscopes.htm&type=5) to define granular permissions for their application users.
...
- Under the **OAuth** section of the Arcade Dashboard left-side menu then click **Providers**.
- Click **Add OAuth Provider** in the top right corner.
- Select the **Custom Provider** tab at the top.

#### Enter the provider details

- Enter an **ID** for your provider with an optional **Description**.
  - If you are using the [Arcade Salesforce toolkit](/toolkits/sales/salesforce), set the ID to `arcade-salesforce`
  - If you are developing your own [custom tools](/home/build-tools/create-a-toolkit) or only using Arcade to authorize users and call Salesforce APIs directly, set the ID to a unique value of your choice (e.g. `my-salesforce-provider`)
...
<Note>
  If you are using the [Arcade Salesforce toolkit](/toolkits/sales/salesforce), the `provider_id` must be set to `arcade-salesforce`. In case you are developing your own [custom tools](/home/build-tools/create-a-toolkit) or only using Arcade to authorize users and call Salesforce APIs directly, the `provider_id` can be set to a unique value of your choice (e.g. `my-salesforce-provider`).
</Note>
