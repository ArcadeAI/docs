...
Arcade Engine manages auth for [tools](/home/auth/auth-tool-calling) and [agents](/home/auth/get-a-token-for-a-user). This configuration controls what providers are available, and how tokens are stored.
...
The `auth.providers` section defines the providers that users can authorize with. Arcade Engine supports many built-in [auth providers](/home/auth-providers), and can also connect to any [OAuth 2.0](/home/auth-providers/oauth2)-compatible authorization server.
...
1. For [built-in providers](/home/auth-providers), use the `provider_id` field to reference them by name. For example:

```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L33-L40 {6}

```

2. For custom providers, specify the connection details using the relevant protocol-specific fields, such as `oauth2`. For full documentation on the custom provider configuration, see the [OAuth 2.0 provider configuration](/home/auth-providers/oauth2) page.

```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L42-L49 {7-8}
