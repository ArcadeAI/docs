...
- `api.development` _(optional, default: `false`)_ - Enable development mode, with more logging and simple [actor authentication](/home/configure/engine#http-actor-configuration)
...
When client code calls the Arcade [LLM API](/home/use-tools/call-tools-with-llms), it specifies a model by name. Arcade Engine matches the model name to a model in the configuration and routes the request to the appropriate provider.
...
Arcade Engine orchestrates [tools](/home/tools-overview) that AI models can use. Tools are executed by distributed workers called **actors**, which are grouped into **directors**.
...
This is the minimal working engine config to get started with Arcade.

**engine-config.yaml**
```yaml file=<rootDir>/examples/code/home/configuration/engine/minimal_config.yaml
