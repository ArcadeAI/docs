...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L3-L6
...
Arcade Engine manages auth for [tools](/home/auth/auth-tool-calling) and [agents](/home/auth/get-a-token-for-a-user). The `auth.providers` section defines the providers that users can authorize with.

Arcade Engine supports many built-in [auth providers](/home/auth-providers), and can also connect to any [OAuth 2.0](/home/auth-providers/oauth2)-compatible authorization server.

The `providers` array contains provider definitions, each which must have a unique `id` within this Arcade Engine instance. There are two ways to configure a provider:
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L8-L16 {7}
...
Additionally, two cache implementations are available:
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L73-L78 {3-6}
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L80-L88
...
At least one root key must be specified. The `default` field indicates which key is used by default:

```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L90-L95
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L98-L105
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L107-L111
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L113-L124 {5-12}
