...
* **LLM chooses among multiple tools**: In the example we followed above, only a single tool was used: `Google.SendEmail`. Check out [this guide](/home/use-tools/call-tools-with-models) where we provide multiple tools for the LLM to choose from. It also features a simple multi-turn conversation loop.
