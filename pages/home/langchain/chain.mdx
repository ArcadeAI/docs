...
You may notice that `_` and `.` are used interchangeably to separate the toolkit name and the tool name. This is due to the fact that some language models do not allow `.` in tool names. Given this, Arcade AI allows you to use either `_` or `.` in tool names interchangeably.
