---
title: "Use Arcade Tools with LangChain"
description: "Integrate Arcade tools into your LangChain chains"
---
...
In this guide, we'll walk through how to use Arcade tools with LangChain to build powerful AI applications.
...
The `ArcadeToolManager` helps you fetch and manage tools from Arcade. Initialize it with your Arcade API key:
...
You may notice that `_` and `.` are used interchangeably to separate the toolkit name and the tool name. This is due to the fact that some language models do not allow `.` in tool names. Given this, Arcade allows you to use either `_` or `.` in tool names interchangeably.
...
Now that you've set up an agent with Arcade tools, you can:
