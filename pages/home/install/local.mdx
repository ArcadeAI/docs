...
<Steps>
### Install the Client
To connect to the cloud or local engine, we need to install the arcade-ai SDK and CLI from the `arcade-ai` package.
...
Note that the Engine requires connection to at least one worker to run. Running the engine on it's own should only be used if external workers are available.
