...
Copy the `arcade.env` and `engine.yaml` files from the [Configuration Templates](/home/configure/templates) into your `$HOME/.arcade/` directory.
...
arcade-engine
```
Note that the Engine requires connection to at least one actor to run. Running the engine on it's own should only be used
if external actors are available.
...
- **Building Tools**: Learn how to build tools with your local Arcade Instance [Create a Toolkit](/home/build-tools/create-a-toolkit) !!!!Theres a disconnect here between using arcade dev and building your own actor
