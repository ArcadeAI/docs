...
<Tabs
  items={["macOS (Homebrew)", "Ubuntu/Debian (APT)", "Other Operating Systems"]}
>
  <Tabs.Tab>```bash brew install ArcadeAI/tap/arcade-engine ```</Tabs.Tab>
  <Tabs.Tab>
    ```bash wget -qO - https://deb.arcade.dev/public-key.asc | sudo apt-key add
    - echo "deb https://deb.arcade.dev/ubuntu stable main" | sudo tee
    /etc/apt/sources.list.d/arcade-ai.list sudo apt update sudo apt install
    arcade-engine ```
  </Tabs.Tab>
  <Tabs.Tab>
    For other operating systems, download the appropriate binary from the
    [Arcade Releases](https://arcade.dev/releases) page and add it to your
    system's `PATH`.
  </Tabs.Tab>
</Tabs>
...
Note that the Engine requires connection to at least one Arcade worker to run. To start the Engine and worker together locally, use the `dev` command:
