...
## Hosted Toolkits
To add a toolkit to a hosted actor such as FastAPI, you can register them in the actor itself. This allows you to explicitly define which tools should be
included on a particular actor.
...
For a full example, see the [Building an actor with FastAPI](/home/serve-tools/build-an-actor-with-fastapi)
...
will show all tools hosted on Arcade Cloud
...
To see all toolkits in Arcade Cloud, run:
```bash
arcade show -h api.arcade-ai.com
```
