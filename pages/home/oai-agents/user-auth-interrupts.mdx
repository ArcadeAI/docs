...
```python
from arcadepy.auth import wait_for_auth, is_authorized
import asyncio
...
Here's a full example that demonstrates the authorization flow:
...
When run for the first time, this will output:

```
Please Login to Github: Authorization required: https://github.com/login/oauth/authorize...
```

After visiting the URL and authorizing, running the same script again will succeed:
