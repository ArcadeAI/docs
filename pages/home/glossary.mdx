...
[Optimized tools](/home/use-tools/types-of-tools#optimized-tools) are hand-crafted tools designed to match the data models and interaction patterns expected in AI-powered conversations. These tools undergo rigorous evaluation to ensure reliable LLM performance and are optimized for speed, accuracy, and cost-effectiveness.

#### Starter tools

[Starter tools](/home/use-tools/types-of-tools#starter-tools) are auto-generated tools that closely mirror the original HTTP API endpoints of upstream services. While they include LLM-friendly names and descriptions, they retain the underlying API structure and require careful prompt engineering and testing before production use.
