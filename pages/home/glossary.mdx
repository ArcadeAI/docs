...
[API proxy tools](/home/use-tools/types-of-tools#api-proxy-tools) are designed to mirror the original API design of the upstream service. They are not optimized for LLM usage and are not subject to evaluation suites. We recommend thoroughly evaluating each API proxy tool with your Agents or chatbots before using it in production.
