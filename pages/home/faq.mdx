...
### Can I add multiple providers of the same type?

Yes, as long as they use a unique ID, you can add as many providers as you need. Arcade's tools out of the box will be routed to a custom provider of the same type, if one exists, or the default arcade-provided provider otherwise. When you add multiple custom providers of the same type, Arcade will not know which one to use and authorizing the tool will fail. To work around this, you will need to manually [define your authorized tools](/home/build-tools/create-a-tool-with-auth#define-your-authorized-tool) to work with this provider by specifying the `id` of the provider.
