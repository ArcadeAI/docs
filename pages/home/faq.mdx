...
Yes, as long as they use a unique ID, you can add as many providers as you need. An important detail is that you will need to manually [define your authorized tools](/home/build-tools/create-a-tool-with-auth#define-your-authorized-tool) to work with this provider. Built-in tools will be routed to the provider that matches the one that Arcade itself provides. For example, if your app relies on our built-in [Gmail Toolkit](/toolkits/productivity/google/gmail), ensure one of your added *Google* providers is named `google`.
