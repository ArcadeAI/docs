...
The `tools.directors` section configures what actors are available to service tool calls:

```yaml file=<rootDir>/examples/code/home/configuration/engine/full_config.yaml#L15-L26

```

When an actor is added to an enabled director, all of the tools hosted by that actor will be available to the LLM and in the Arcade API.
...
- `timeout` _(default: `30s`)_ - Timeout for calling the actor's tools
- `retry` _(default: `3`)_ - Number of retries to attempt
...
If `api.development = true`, the secret will default to `"dev"` for local development **only**. In production, the secret must be set to a non-default, random value.
