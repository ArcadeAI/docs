...
- [`telemetry`](#telemetry) - Configures telemetry and observability
...
- `api.development` _(default: `false`)_ - Enable development mode, with more logging
- `api.http.host` _(default: `localhost`)_ - Address to which Arcade Engine binds its server (e.g., `localhost` or `0.0.0.0`)
- `api.http.read_timeout` _(default: `30s`)_ - Timeout for reading data from clients
- `api.http.write_timeout` _(default: `1m`)_ - Timeout for writing data to clients
- `api.http.idle_timeout` _(default: `30s`)_ - Timeout for idle connections
- `api.http.max_request_body_size` _(default: `4Mb`)_ - Maximum request body size
...
The `llm.models` section defines the models that the engine can route to, and the parameters for each model. Each item in the `models` array must have a unique `id` and a provider-specific configuration.
...
For more details on model configuration and provider-specific parameters, see [model configuration](/home/configuration/models).

### Routing

When client code calls the Arcade [LLM API](/guides/llmtoolcalling), it specifies a model by name. Arcade Engine matches the model name to a model in the configuration and routes the request to the appropriate provider.
...
For example, given the above `llm.models` configuration with one provider named `primary`, this request will be routed to `primary` with the model `gpt-4o`:
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/auth_tokenstore.redis.yaml
...
```yaml file=<rootDir>/examples/code/home/configuration/engine/auth_providers.yaml
