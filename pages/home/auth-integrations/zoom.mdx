...
- Your [app code](#using-zoom-auth-in-your-app-code) that needs to call Zoom APIs
- Or, your [custom tools](#using-zoom-auth-in-customtools) that need to call Zoom APIs

## Configuring Zoom auth

How you configure the Zoom auth provider depends on whether you use the Arcade [Cloud Engine](/home/cloud-engine) or a [self-hosted Engine](/guides/installation).
...
Use the Zoom auth provider in your own agents and AI apps to get a user token for the Zoom API. See [authorizing agents with Arcade](/guides/agentauth) to understand how this works.

Use `client.auth.authorize()` to get a user token for the Zoom API:

```python file=<rootDir>/examples/code/integrations/zoom/custom_auth.py {9-13}

```

## Using Zoom auth in custom tools

The Arcade [LLM API](/guides/llmtoolcalling) is a convenient way to call LLMs and automatically invoke tools. You can author your own [custom tools](/guides/customtools) that interact with the Zoom API.
...
```python file=<rootDir>/examples/code/integrations/zoom/custom_tool.py {7,10-14,24}
