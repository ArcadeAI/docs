...
describe("Clean Markdown Files", () => {
  ...
  test(
    "no orphaned markdown files (all have corresponding MDX sources)",
    async () => {
      // Get all MDX source files
      const mdxFiles = await fg("app/**/page.mdx", {
        ignore: [
          "app/_*/**",
          // Skip dynamic toolkit routes — they render from JSON, not MDX
      ...
      // Build set of expected markdown paths
      const expectedMarkdownPaths = new Set(
        mdxFiles.map((mdxFile) => {
          const relativePath = mdxPathToRelativePath(mdxFile);
          return path.join(MARKDOWN_DIR, `${relativePath}.md`);
        })
      );
      ...
      // Find orphans (excluding toolkit-generated integration files)
      const orphaned = markdownFiles.filter(
        (mdFile) =>
          !(
            expectedMarkdownPaths.has(mdFile) ||
            TOOLKIT_INTEGRATION_PATTERN.test(mdFile)
          )
      );

      if (orphaned.length > 0) {
        console.error(
          "\nOrphaned markdown files (no corresponding MDX source):"
        );
        for (const file of orphaned) {
          console.error(`  - ${file}`);
        }
        console.error(
          "\nRun 'pnpm generate:markdown' to clean up orphaned files."
        );
      }

      expect(
        orphaned.length,
        `${orphaned.length} markdown files are orphaned (no MDX source)`
      ).toBe(0);
    },
    TIMEOUT
  );
