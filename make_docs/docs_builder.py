...
async def build_examples(
    ...
    async def build_with_semaphore(tool: ToolDefinition):
        async with semaphore:
            return await build_tool_examples(print_debug, tool, openai_model)

    responses = await asyncio.gather(*[build_with_semaphore(tool) for tool in tools])
...
async def build_tool_examples(
    ...
    input_map = await generate_tool_input_map(interface_signature, openai_model)
...
async def generate_tool_input_map(
    ...
    text = await async_request_openai_generation(
        model=openai_model, max_tokens=512, messages=messages
    )
    ...
    try:
        ...
        if retries < max_retries:
            return await generate_tool_input_map(
...
async def async_request_openai_generation(
    model: str,
    max_tokens: int,
    messages: list[dict[str, Any]],
) -> str:
    # Create async client instance using the API key from the global openai module
    async_client = AsyncOpenAI(api_key=openai.api_key)

    if model.startswith("gpt-5"):
        # Note: GPT-5 API might not be available yet, this is placeholder code
