...
<ToolInfo
  ...
  authType="cadena de conexión a la base de datos"
  versions={["0.1.0"]}
/>

<Badges repo="arcadeai/arcade-mongodb" />

El kit de herramientas de MongoDB de Arcade proporciona un conjunto preconstruido de herramientas para interactuar con bases de datos MongoDB de manera de solo lectura. Este kit de herramientas permite a los agentes descubrir bases de datos y colecciones, explorar estructuras de documentos y ejecutar consultas de forma segura. Este kit de herramientas es un complemento de la publicación del blog [Diseñando herramientas SQL para agentes de IA](https://blog.arcade.dev/sql-tools-ai-agents-security).

<Callout type="info">
Este kit de herramientas está destinado a ser un ejemplo de cómo construir un kit de herramientas para una base de datos, y no está destinado a ser utilizado en producción; no lo encontrarás listado en el panel de Arcade o en las APIs. Para uso en producción, recomendamos bifurcar este repositorio y construir tu propio kit de herramientas con herramientas específicas para el caso de uso.
</Callout>
...
* **Inferencia de Esquema**: Muestra documentos para inferir la estructura del esquema y los tipos de datos
...
* **Agrupamiento de Conexiones**: Reutilizar conexiones a la base de datos de manera eficiente
...
<TableOfContents
  ...
  data={
    [
      ['MongoDB.DiscoverDatabases', "Descubrir todas las bases de datos en la instancia de MongoDB."],
      ['MongoDB.DiscoverCollections', "Descubrir todas las colecciones en una base de datos específica."],
      ['MongoDB.GetCollectionSchema', "Obtener la estructura del esquema de una colección muestreando documentos."],
      ['MongoDB.FindDocuments', "Encontrar documentos en una colección con filtrado, proyección y ordenación."],
      ['MongoDB.CountDocuments', "Contar documentos que coinciden con un filtro específico."],
      ['MongoDB.AggregateDocuments', "Ejecutar tuberías de agregación para análisis de datos complejos."],
    ]
  }
/>
...
Descubre todas las bases de datos en la instancia de MongoDB. Esta herramienta devuelve una lista de todas las bases de datos disponibles, excluyendo bases de datos del sistema como `admin`, `config` y `local` por razones de seguridad.
...
Descubre todas las colecciones en una base de datos específica. Esta herramienta debe ser utilizada antes de cualquier otra herramienta que requiera un nombre de colección.
...
Obtén la estructura del esquema de una colección muestreando documentos. Dado que MongoDB no tiene esquema, esta herramienta muestrea un número configurable de documentos para inferir la estructura del esquema y los tipos de datos. Siempre usa esta herramienta antes de ejecutar cualquier consulta.
...
Encuentra documentos en una colección con filtrado, proyección y ordenación. Esta herramienta te permite construir consultas complejas utilizando los operadores de consulta de MongoDB mientras mantienes la seguridad y el rendimiento.
...
- `sort` (list[str], opcional): Criterios de ordenación como lista de cadenas JSON con claves 'field' y 'direction'
...
- Siempre especifica la proyección para limitar los campos devueltos si no necesitas todos los datos
...
Cuenta documentos en una colección que coinciden con el filtro dado. Esta herramienta es útil para obtener conteos rápidos sin recuperar los documentos reales.
...
Ejecuta tuberías de agregación para análisis de datos complejos. Esta herramienta te permite ejecutar operaciones de procesamiento de datos sofisticadas, incluyendo agrupación, filtrado y transformaciones.

**Parámetros:**
- `database_name` (str): El nombre de la base de datos a consultar
- `collection_name` (str): El nombre de la colección a consultar
- `pipeline` (list[str]): Tubería de agregación de MongoDB como lista de cadenas JSON
...
- `$lookup` - unirse a otras colecciones
