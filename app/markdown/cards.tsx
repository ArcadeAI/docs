...
export const Card = ({ title, children, footer, icon, color, href }: Props) => {
  ...
  const cardContent = (
    <>
      ...
      {title && <div className="mt-2 mb-2 font-bold text-xl">{title}</div>}
      <div>{children}</div>
  ...
  return href ? (
    <Link
      className={cn(
        wrapperClasses,
        borderColorVariations.accent,
        'hover:_bg-[var(--brand-accent-hover)] cursor-pointer'
      )}
      href={href}
      passHref
    >
    ...
    <div className={cn(wrapperClasses, borderColorVariations.grey)}>
      {cardContent}
    </div>
  );
};
