...
<Tabs items={['macOS (Homebrew)', 'Ubuntu/Debian (APT)', 'Manual Download']}>
  ...
  ```
  <Note>To manually download the engine.yaml, you can get an example from the [Configuration Templates](/home/hosting-options/local-deployment/configure/templates#engineyaml) and add it to `$HOME/.arcade/engine.yaml`.</Note>
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['macOS (Homebrew)', 'Ubuntu/Debian (APT)', 'Manual Download']}>
  ...
  ```
  <Note>To manually download the `engine.env`, refer to the [Configuration Templates](/home/hosting-options/local-deployment/configure/templates#engineenv).</Note>
  </Tabs.Tab>
</Tabs>
