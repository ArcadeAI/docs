...
<Note>
  At this time, Arcade does not offer a default Salesforce Auth Provider and
  cannot support Salesforce auth in the Arcade Cloud. To use Salesforce auth,
  the [Salesforce toolkit](/toolkits/categories/sales/salesforce), or to develop
  your [custom Salesforce
  tools](/home/authoring-tools/build-tools/create-a-toolkit), you must
  [self-host the Arcade
  Engine](/home/hosting-options/local-deployment/install/local) and create a
  custom Auth Provider with your own Salesforce OAuth 2.0 credentials as
  described below.
</Note>
...
- The [Arcade Salesforce toolkit](/toolkits/categories/sales/salesforce), which provides pre-built tools for interacting with Salesforce services
...
The custom scopes required by the [Arcade Salesforce toolkit](/toolkits/categories/sales/salesforce) are listed below, along with their descriptions:

<Note>
  The custom scopes listed below are only required if you are using the [Arcade Salesforce toolkit](/toolkits/categories/sales/salesforce).

If you're creating your own [custom Salesforce tools](/home/authoring-tools/build-tools/create-a-toolkit) or using Arcade to authorize users and call Salesforce APIs directly, you are free to define custom scope(s) that fit best your application use cases. Observe that you must have at least one custom scope assigned to your Salesforce app in order to use the Salesforce API.
...
<Note>
  Refer to [Installing Arcade
  Locally](/home/hosting-options/local-deployment/install/local) for more
  information on how to install and run the Arcade Engine.
</Note>
...
- Enter `salesforce` as the **ID** for your provider (the ID must be `salesforce` in order to use the [Arcade Salesforce toolkit](/toolkits/categories/sales/salesforce)).
...
<Tip>
  Refer to [Engine
  configuration](/home/hosting-options/local-deployment/configure/engine) for
...
<Tip>
  ...
  file](/home/hosting-options/local-deployment/configure/overview#engine-configuration-file)
  documentation.
</Tip>
...
<Note>
  ...
  toolkit](/toolkits/categories/sales/salesforce).
</Note>
...
The [Arcade Salesforce toolkit](/toolkits/categories/sales/salesforce) provides tools to interact with various Salesforce objects, such as accounts, contacts, leads, opportunities, notes, tasks, email messages, call logs, etc.

Refer to the [toolkit documentation and examples](/toolkits/categories/sales/salesforce) to learn how to use the toolkit to build agents and AI apps that interact with Salesforce services.

<Tip>
  Check our introductory documentation to understand what are tools and how
  [tool calling works](/home/core-concepts/use-tools/tools-overview).
</Tip>

## Calling Salesforce APIs directly

Use the Salesforce auth provider to get a user authorization token and call Salesforce API endpoints directly, without the use of any tools. See [How Arcade helps with Agent Authorization](/home/core-concepts/auth/how-arcade-helps) to understand how this works.

### Prerequisites

1. Create an <SignupLink linkLocation="docs:/home/hosting-options/auth-providers/salesforce">Arcade account</SignupLink>
1. Get an [Arcade API key](/home/getting-started/api-keys).
...
<Note>Replace `http://localhost:9099` with the URL of your Arcade Engine.</Note>
...
If the pre-built tools in the [Arcade Salesforce toolkit](/toolkits/categories/sales/salesforce) don't meet your needs, you can create your own [custom tools](/home/authoring-tools/build-tools/create-a-toolkit) that interact with the Salesforce APIs.
