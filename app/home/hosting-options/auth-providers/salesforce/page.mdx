...
<Note>
  At this time, Arcade does not offer a default Salesforce Auth Provider and cannot support Salesforce auth in the Arcade Cloud. To use Salesforce auth, the [Salesforce toolkit](/toolkits/categories/sales/salesforce), or to develop your [custom Salesforce tools](/home/authoring-tools/build-tools/create-a-toolkit), you must [self-host the Arcade Engine](/home/hosting-options/local-deployment/install/local) and create a custom Auth Provider with your own Salesforce OAuth 2.0 credentials as described below.
</Note>
...
<Note>
  Refer to [Installing Arcade Locally](/home/hosting-options/local-deployment/install/local) for more information on how to install and run the Arcade Engine.
</Note>
...
<Tip>
  Refer to [Engine configuration](/home/hosting-options/local-deployment/configure/engine) for more information on how to set environment variables and configure the Arcade Engine.
</Tip>

<Tip>
  To find where the `engine.yaml` file is located in your OS after installing the Arcade Engine, check the [Engine configuration file](/home/hosting-options/local-deployment/configure/overview#engine-configuration-file) documentation.
</Tip>
...
<Note>
  The `id` must be set to `salesforce` in order to use the [Arcade Salesforce toolkit](/toolkits/categories/sales/salesforce).
</Note>
...
<Tip>
  Check our introductory documentation to understand what are tools and how [tool calling works](/home/core-concepts/use-tools/tools-overview).
</Tip>
