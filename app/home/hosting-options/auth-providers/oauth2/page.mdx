...
<Tip>
  Arcade has [pre-built integrations](/home/hosting-options/auth-providers) with many popular
...
How you configure the OAuth 2.0 provider depends on whether you use the Arcade Cloud Engine or a [self-hosted Engine](/home/hosting-options/local-deployment/install/overview). If you use the Cloud Engine, you must configure your provider in the Dashboard.
...
<Tip>
  When using the Arcade Cloud, the Dashboard is available at [`https://api.arcade.dev/dashboard`](https://api.arcade.dev/dashboard). If you are [self-hosting Arcade](/home/hosting-options/local-deployment/install/overview), by default the Dashboard is available at [`http://localhost:9099/dashboard`](http://localhost:9099/dashboard). Adjust the host and port, if necessary, to match your environment.
</Tip>
...
<Note>
This method is only available when you are [self-hosting the engine](/home/hosting-options/local-deployment/install/overview)
</Note>
...
<Tip>
  See [configuration](/home/hosting-options/local-deployment/configure/engine) for more information on how to set
...
<Tip>
  To locate the `engine.yaml` file in your OS after installing the Arcade Engine, check the [Engine configuration file](/home/hosting-options/local-deployment/configure/overview#engine-configuration-file) documentation.
</Tip>
...
Use the OAuth 2.0 auth provider in your own agents and AI apps to get a user token for any OAuth 2.0-compatible APIs. See [authorizing agents with Arcade](/home/core-concepts/auth/how-arcade-helps) to understand how this works.
...
You can author your own [custom tools](/home/authoring-tools/build-tools/create-a-toolkit) that interact with any OAuth 2.0-compatible APIs.
