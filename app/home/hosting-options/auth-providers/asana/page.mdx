...
<Tip>
  ...
  pre-built [Arcade Asana toolkit](/toolkits/categories/productivity/asana) is what you
  want!
</Tip>
...
- The [Arcade Asana toolkit](/toolkits/categories/productivity/asana), which provides pre-built tools for interacting with Asana
...
If you choose to use Arcade's Asana auth, you don't need to configure anything. Follow the [Asana toolkit examples](/toolkits/categories/productivity/asana) to get started calling Asana tools.
...
You can use your own Asana credentials in both the Arcade Cloud and in a [self-hosted Arcade Engine](/home/hosting-options/local-deployment/install/local) instance.
...
<Tip>
  Refer to [Engine configuration](/home/hosting-options/local-deployment/configure/engine) for more information
...
<Tip>
  ...
  file](/home/hosting-options/local-deployment/configure/overview#engine-configuration-file) documentation.
</Tip>
...
The [Arcade Asana toolkit](/toolkits/categories/productivity/asana) provides tools to interact with various Asana objects, such as tasks, projects, teams, and users.

Refer to the [toolkit documentation and examples](/toolkits/categories/productivity/asana) to learn how to use the toolkit to build agents and AI apps that interact with Asana services.

## Using Asana auth in app code

Use the Asana auth provider in your own agents and AI apps to get a user-scoped token for the Asana API. See [authorizing agents with Arcade](/home/core-concepts/auth/how-arcade-helps) to understand how this works.
...
<Tabs.Tab>
<Note>
  If you are [self-hosting Arcade](/home/hosting-options/local-deployment/install/overview), change the `base_url` parameter in the `Arcade` constructor to match your Arcade Engine URL. By default, the Engine will be available at `http://localhost:9099`.
</Note>
...
<Tabs.Tab>
<Note>
  If you are [self-hosting Arcade](/home/hosting-options/local-deployment/install/overview), change the `baseURL` parameter in the `Arcade` constructor to match your Arcade Engine URL. By default, the Engine will be available at `http://localhost:9099`.
</Note>
...
You can use the pre-built [Arcade Asana toolkit](/toolkits/categories/productivity/asana) to quickly build agents and AI apps that interact with Asana.

If the pre-built tools in the Asana toolkit don't meet your needs, you can author your own [custom tools](/home/authoring-tools/build-tools/create-a-toolkit) that interact with Asana API.
...
If you are [self-hosting Arcade](/home/hosting-options/local-deployment/install/overview), you will need to restart the Arcade Worker and the Engine for the new tool to be available.
