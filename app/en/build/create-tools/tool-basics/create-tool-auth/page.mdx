...
2. If the tool's requirements are not met, Arcade initiates the provider-specific OAuth flow for the requested scopes.
    ...
    > 2b). The provider issues the token, and Arcade will securely inject it into the tool's [`Context`](/build/create-tools/tool-basics/runtime-data-access) on its next invocation. The client and the LLM will never see the token.
...
- Learn how to [build a tool with secrets](/build/create-tools/tool-basics/create-tool-secrets)
