...
Resolve one subscription from your HTTP route before calling `receive`, and pass only that subscription's current and previous secrets as `subscription_secrets`. Before side effects, have the handler allow-list that subscription's event types and compare tenant identifiers with its server-side configuration. The generic verifier does not treat payload identifiers as authority. Configure a request-body size limit in the HTTP framework before reading the raw body.
...
- **Identity:** One delivery keeps the same `webhook-id` across retries. That value is also the `{delivery_id}` used by the delivery detail and retry APIs. Different events, and one event delivered through different subscriptions, receive different IDs.
