...
The sample records `received_at` for that cleanup policy but does not schedule the cleanup job for you.
...
| Webhook subscription | Inspect, enable, disable, delete, recover, replay | `GET /webhooks/{webhook_id}`, `PATCH /webhooks/{webhook_id}`, `DELETE /webhooks/{webhook_id}`, `POST /webhooks/{webhook_id}/recover_deliveries`, `POST /webhooks/{webhook_id}/replay_missing` |
...
- **Signatures:** Verify the raw body, `webhook-id`, and `webhook-timestamp` before deduplication. Timestamps through 300 seconds in either direction are accepted; timestamps 301 seconds away are rejected.
