...
Every MCP server in Arcade has a visibility scope, set on the server rather than on any gateway. A **Project** server is registered to one project, and its tools are then available to every gateway, Playground session, and SDK call in that project; this includes MCP servers you deploy and remote MCP servers you register. A **Global** server is available to all organizations, projects, and accounts; Arcade's pre-built catalog is served this way.
