...
<Callout type="info">
You can mix MCP servers (which give you all their tools) with individual tools. Browse the [complete MCP server catalog](/resources/integrations) to see what's available.
</Callout>
...
const markdownComponents: Components = {
  ...
  pre: ({ children }) => (
    <pre className="bg-gray-700 p-2 rounded overflow-x-auto my-2">{children}</pre>
  ),
};
```
    ...
    const pollStatus = async () => {
      try {
        ...
        if (data.status === "completed" && !hasCompletedRef.current) {
          ...
          setStatus("completed");
          // Wait briefly to show success, then trigger retry
              ...
              <div className={`inline-block p-3 rounded-lg ${
                ...
                {showLoading ? (
                  ...
                  message.parts?.map((part, i) => {
                    ...
                    // Handle tool results that need authorization
                    if (part.type.startsWith("tool-")) {
                      const toolPart = part as { state?: string; output?: unknown };
...
2. **Add user authentication**: In production, get `userId` from your auth system (NextAuth, Clerk, etc.) instead of environment variables.
3. **Customize the UI**: Add markdown rendering, better styling, or integrate with your design system.
