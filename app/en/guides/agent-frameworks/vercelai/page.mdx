...
[AI Elements](https://ai-sdk.dev/elements) provides pre-built React components for chat interfaces with auto-scrolling, markdown rendering, and loading states built in.
...
Follow the prompts to complete the installation. AI Elements will create a `components/` directory with UI components.

<Callout type="info">
AI Elements installs components directly into your codebase, giving you full control to customize them later.
</Callout>
...
function toVercelTools(arcadeTools: Record<string, any>): Record<string, any> {
  ...
  for (const [name, tool] of Object.entries(arcadeTools)) {
    ...
    vercelTools[name] = {
      ...
      inputSchema: tool.parameters,  // AI SDK v6 uses inputSchema, not parameters
...
The `Conversation` component handles auto-scrolling, `Message` handles role-based styling, and `MessageResponse` renders markdown. We just need to check for Arcade's `authorization_required` flag in tool results:
              ...
              <Message key={message.id} from={message.role}>
                <MessageContent>
                  ...
                  {message.role === "assistant" && !textContent && !authPart && isLoading ? (
                    ...
                    (() => {
                      const toolPart = authPart as { output?: unknown };
                      const result = toolPart.output as Record<string, unknown>;
                      const authResponse = result?.authorization_response as { url?: string };
                      const toolName = authPart.type.replace("tool-", "");
                      return (
                        <AuthPendingUI
