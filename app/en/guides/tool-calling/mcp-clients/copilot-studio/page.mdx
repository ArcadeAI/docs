...
In Copilot Studio, create a new agent or open an existing one that you want to connect to Arcade tools.

### Add a new MCP tool

1. Inside your agent, click on the **Tools** tab in the navigation panel
...
1. Enter a **Server name** for your connection (e.g., "PersonalAssistantTools")
...
Arcade provides just-in-time authorization—when you use a tool that requires access to an external service, Copilot Studio will display an authorization link. Simply click the link to grant access and continue. This works seamlessly with tools like SharePoint, Outlook, Teams, Stripe, and Gmail.
