...
## Get all tool definitions in an MCP server formatted for a model

You can list tool definitions for an MCP server in a specific model provider's format. For example, you may want to get the definitions of tools in the `Github` MCP server in OpenAI's format.

Use the `client.tools.formatted.list` method and specify the MCP server and format. Since this method returns an iterator of pages, you can cast to a list to get all the tools.
...
To get all tools formatted for OpenAI, use the `client.tools.formatted.list` method without specifying an MCP server.
...
2. **TypeScript Integration**: Provides outstanding TypeScript support with automatic type inference
...
This approach improves your code by:
