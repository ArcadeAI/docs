...
You could go to your Drive/Docs, open the document, copy its contents, and paste it into your chat. But what if the LLM could do this for you? The Arcade Google Docs MCP Server provides a [`SearchAndRetrieveDocuments`](/en/resources/integrations/productivity/googledocs#googledocssearchandretrievedocuments) tool. By calling it, the LLM can find and read the document without you having to do anything.

After analyzing the document, you decide that a meeting is needed with your colleague. You can ask the LLM to schedule a meeting and it will use the [Google Calendar MCP Server](/en/resources/integrations/productivity/googlecalendar) to do it without you needing to leave the chat.

Or you could ask the LLM to send a summary of the analysis to your colleague by email and it would use the [Gmail MCP Server](/en/resources/integrations/productivity/gmail) for that.
...
Arcade solves this problem by providing a standardized [interface for authorization](/en/get-started/about-arcade), as well as pre-built auth providers for [popular services](/en/references/auth-providers) such as Google, Dropbox, GitHub, Notion, and many more.

Our SDK also [allows you to integrate](/en/references/auth-providers/oauth2) LLMs with any OAuth 2.0-compliant API.
...
- Explore the [MCP Servers](/en/resources/integrations) available on Arcade
- Build your own [custom MCP Server](/en/guides/create-tools/tool-basics/build-mcp-server)
