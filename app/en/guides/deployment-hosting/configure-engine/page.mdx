...
This page explains how to configure the Arcade Engine for self-hosted deployments.

<Callout type="warning">
  This page is for enterprise customers who are self-hosting the Arcade Engine. This page contains advanced configuration options that are not applicable for most customers.
</Callout>
...
This means that you cannot find the Arcade Engine in your path. Brew and Apt will automatically add the binary to your path.

Check that you have properly installed the binary. These are the common installation locations):
...
If you find the binary, add it to your path with:
...
```
  ...
  <Tabs.Tab>
  You can pull the docker image for the engine with
...
When you install Arcade, it creates configuration files to manage engine settings and default values. The installation creates two files:
- The `engine.yaml` file for engine configuration.
- The `engine.env` file for environment variables.
Explore each file to understand their purpose and how to locate them.

## Engine configuration file

The `engine.yaml` file controls Arcade Engine settings. It supports variable expansion so you can integrate secrets and environment values. You can customize this file to suit your setup. For more details, check the [Engine Configuration](/guides/deployment-hosting/configure-engine) page.
...
<Tabs items={['macOS (Homebrew)', 'Ubuntu/Debian (APT)', 'Manual Download']}>
  ...
  ```
  <Callout type="info">To manually download the engine.yaml, you can get an example from the [Configuration Templates](/guides/deployment-hosting/configure-engine#engineyaml) and add it to `$HOME/.arcade/engine.yaml`.</Callout>
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['macOS (Homebrew)', 'Ubuntu/Debian (APT)', 'Manual Download']}>
  ...
  ```
  <Callout type="info">To manually download the `engine.env`, refer to the [Configuration Templates](/guides/deployment-hosting/configure-engine#engineenv).</Callout>
  </Tabs.Tab>
</Tabs>
...
Arcade Engine automatically loads environment variables from `.env` files in the directory where it called the executable. Use the `-e` or`--env` flag to specify a path:
...
- `api.host` _(default: `localhost`)_ - Address to which Arcade Engine binds its server (for example, `localhost` or `0.0.0.0`)
- `api.port` _(default: `9099`)_ - Port to which Arcade Engine binds its server (for example, `9099` or `8080`)
- `api.public_host` _(optional)_ - External hostname of the API (for example, `my-public-host.com`), if it differs from `api.host` (for example, when Arcade Engine is behind a reverse proxy)
- `api.read_timeout` _(optional, default: `30 s`)_ - Timeout for reading data from clients
- `api.write_timeout` _(optional, default: `1m`)_ - Timeout for writing data to clients
- `api.idle_timeout` _(optional, default: `30 s`)_ - Timeout for idle connections
...
When a container hosts the Arcade Engine or places it behind a reverse proxy, set `api.public_host` to the external hostname of the API:
...
You can configure auth for [AI tools](/guides/tool-calling/custom-apps/auth-tool-calling) and [direct API calls](/guides/tool-calling/call-third-party-apis) through Arcade Engine. It supports many built-in [auth providers](/references/auth-providers), and can also connect to any [OAuth 2.0](/references/auth-providers/oauth2) authorization server.

The `auth.providers` section defines the providers that users can authorize with. Each provider must have a unique `id` in the array. You can configure a provider in two ways:
...
- `api_key_ttl` _(optional, default: `10 s`)_ - The time-to-live for API keys in the cache
...
The `security` section configures the root encryption keys that Arcade Engine uses to encrypt and decrypt data at rest. See the [storage configuration](#storage-configuration) section below to configure where the system stores data.
...
When you [install Arcade Engine locally](/guides/deployment-hosting/configure-engine), the system creates an `engine.env` file with a default root key:
...
You can only use this default value in development mode (see [API configuration](#api-configuration) above).
...
The `storage` section configures the storage backend that Arcade Engine uses to store persistent data.

These storage implementations are available:
...
If you use the Arcade Engine locally, you can set the `environment` field to `local`. This will only output logs to the console:
...
`environment` and `version` are fields that the system adds to the telemetry attributes, which you can filter on later.
...
When the system adds an MCP server to an enabled director, all of the tools that MCP server hosts will be available to the model and through the Arcade API.

### HTTP MCP server configuration

The `http` sub-section configures the HTTP client that you use to call the MCP server's tools:

- `uri` _(required)_ - The base URL of the MCP server's tools
- `secret` _(required)_ - Secret that you use to authenticate with the MCP server
- `timeout` _(optional, default: `30 s`)_ - Timeout for calling the MCP server's tools
- `retry` _(optional, default: `3`)_ - Number of times to retry a failed tool call

<Callout type="info">
  You must configure MCP servers with a `secret` to authenticate with
  the MCP server. This ensures that MCP servers are not exposed to the public internet
  without security.

If `api.development = true`, the secret will default to `"dev"` for local development **only**. In production, you must set the secret to a random value.
