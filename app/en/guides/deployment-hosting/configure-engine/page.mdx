...
```
  ...
  where config.yaml is the path to the [configuration file](/en/guides/deployment-hosting/configure-engine).
  </Tabs.Tab>
</Tabs>

The Homebrew tab is macOS-only. For native Windows environments, use Docker for
Arcade Engine. For Arcade CLI setup on Windows, see [Windows environment setup](/en/get-started/setup/windows-environment).
...
The `engine.yaml` file controls Arcade Engine settings. It supports variable expansion so you can integrate secrets and environment values seamlessly. You can customize this file to suit your setup. For more details, check the [Engine Configuration](/en/guides/deployment-hosting/configure-engine) page.
...
<Tabs items={['macOS (Homebrew)', 'Ubuntu/Debian (APT)', 'Manual Download']}>
  ...
  ```
  <Callout type="info">To manually download the engine.yaml, you can get an example from the [Configuration Templates](/en/guides/deployment-hosting/configure-engine#engineyaml) and add it to `$HOME/.arcade/engine.yaml`.</Callout>
  </Tabs.Tab>
</Tabs>
...
<Tabs items={['macOS (Homebrew)', 'Ubuntu/Debian (APT)', 'Manual Download']}>
  ...
  ```
  <Callout type="info">To manually download the `engine.env`, refer to the [Configuration Templates](/en/guides/deployment-hosting/configure-engine#engineenv).</Callout>
  </Tabs.Tab>
</Tabs>
...
Arcade Engine manages auth for [AI tools](/en/guides/tool-calling/custom-apps/auth-tool-calling) and [direct API calls](/en/guides/tool-calling/call-third-party-apis). It supports many built-in [auth providers](/en/references/auth-providers), and can also connect to any [OAuth 2.0](/en/references/auth-providers/oauth2) authorization server.
...
For [built-in providers](/en/references/auth-providers), use the `provider_id` field to reference the pre-built configuration. For example:
...
For custom OAuth 2.0 providers, specify the full connection details in the `oauth2` sub-section. For full documentation on the custom provider configuration, see the [OAuth 2.0 provider configuration](/en/references/auth-providers/oauth2) page.
...
When you [install Arcade Engine locally](/en/guides/deployment-hosting/configure-engine), an `engine.env` file is created with a default root key:
...
Arcade Engine orchestrates [tools](/en/guides/tool-calling) that AI models can use.
