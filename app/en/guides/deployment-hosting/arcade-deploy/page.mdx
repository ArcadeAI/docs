...
If you have not created an MCP server yet, then follow the steps outlined in [this guide](/en/guides/create-tools/tool-basics/build-mcp-server) before deploying.
...
- Create [MCP Gateways](/en/guides/mcp-gateways)

## Create an MCP Gateway to call the tools in your MCP Server

Once the MCP server is deployed to Arcade, all the tools in the server will be available in the [tool catalog](https://api.arcade.dev/dashboard/tools) page in your Arcade dashboard. To call the tools from an MCP client, you first need to [create an MCP Gateway](/en/guides/mcp-gateways) to pick and choose which tools you want to use in your MCP clients.
...
You can use any of the available [Arcade clients](/en/references) to call the tools in your MCP Server. When using the clients, you are not required to create an MCP Gateway, as the client will handle the connection to all tools in your Arcade project directly.
