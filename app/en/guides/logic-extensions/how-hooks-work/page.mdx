...
- **Tool filter** — Optional include/exclude patterns to limit which tools trigger this hook
...
Hooks at both organization and project scope run — there is no override. The Engine executes them in this fixed order:
...
Organization hooks have a **phase** setting (`before` or `after`) that controls whether they run before or after project hooks. Project hooks always run in the middle — they do not have a phase setting.
...
{/* TODO: Replace with actual screenshots once the dashboard UI is finalized */}

## Next steps

- [Run an extension](/guides/logic-extensions/examples) — Try the open-source example servers
- [Build your own](/guides/logic-extensions/build-your-own) — Implement the webhook contract from the OpenAPI spec
