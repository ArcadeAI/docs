...
You can implement a webhook server that satisfies the Logic Extensions contract in any language—the contract defines an OpenAPI 3.0 spec.

## OpenAPI spec

The canonical contract resides in the [ArcadeAI/schemas](https://github.com/ArcadeAI/schemas) repository:
...
You can browse it interactively on the [API Reference](/references/contextual-access-webhook-api) page.
...
The [logic-extensions-examples](https://github.com/ArcadeAI/logic-extensions-examples) repo uses this approach—see the examples below.

## Endpoints to implement

Your server needs to expose HTTP POST endpoints for each hook point you want to handle. Endpoint paths are fully configurable in the Dashboard—the examples below show the defaults.

| Endpoint | Hook point | Required |
| --- | --- | --- |
| `POST /access` | Access Hook | Only if you need tool access control |
| `POST /pre` | Pre-Execution Hook | Only if you need tool request validation/modification |
| `POST /post` | Post-Execution Hook | Only if you need tool output filtering/modification |
...
| --- | --- | --- | --- |
| `code` | string | Yes | `OK`, `CHECK_FAILED`, or `RATE_LIMIT_EXCEEDED` |
| `error_message` | string | No | Displayed to the agent when denying |
| `override` | object | No | `inputs` and/or `secrets` to modify the request |
...
| `execution_code` | string | Status from worker |
| `execution_error` | string | Error message from tool call |
| `context` | object | Same as pre-execution hook |
...
| `code` | string | Yes | `OK`, `CHECK_FAILED`, or `RATE_LIMIT_EXCEEDED` |
| `error_message` | string | No | Displayed to the agent when denying |
| `override` | object | No | `output` to replace the response returned to the agent |
...
| Field | Type | Description |
| --- | --- | --- |
| `only` | object | If present, **only** these tools receive access (deny list ignored) |
| `deny` | object | These tools receive denial (ignored if `only` is present) |

If you return neither field, Arcade treats it as "no change"—all tools receive access at this hook.

## Response codes (pre and post)

| Code | Meaning |
| --- | --- |
| `OK` | Proceed; optional `override` applies |
| `CHECK_FAILED` | Deny the operation; the agent sees `error_message` |
...
- **Retries:** Optional at the extension level; only for transient failures (5xx, timeout, connection error). 4xx responses do not retry.
