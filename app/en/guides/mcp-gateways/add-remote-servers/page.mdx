...
MCP Gateways let you manage and filter tools in one place, so your agent or team uses a curated set of tools across clients.  This is especially useful for remote MCP servers, where you may not have control over the tools available.  By using MCP Gateways, you can easily select the tools you want to use and hide the ones you don't want to use.
...
- **ID**: A unique, human-readable identifier (for example, `figma-mcp-server`)
- **URI**: The public URL for your MCP server (for example, `https://mcp.figma.com/mcp`)
...
Create the server and confirm that Arcade lists the server tools in your project.  If the remote MCP server requires authentication, you will be prompted to complete the OAuth flow.

Don't worry, we will always load the tools available per Arcade user, but we pre-load the list of tools so that you can filter them by your own criteria in your MCP Gateways.
...
2. Open **Select tools** and filter by your remote server ID.
...
result = client.tools.execute(
    tool_name="figma-mcp-server.get_user_info",
...
Remote servers must be reachable from Arcade and must support the Streamable HTTP(s) transport. If your server depends on non-HTTP MCP transports, Arcade cannot proxy it.
</Callout>
