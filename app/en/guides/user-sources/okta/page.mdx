...
You don't need to touch the **Advanced** section in the User Source form for a standard Okta setup, but two settings live there for cases where the defaults don't fit:

- **Subject Claim**: defaults to `sub`, Okta's stable user identifier. Switch this only if you've configured a custom claim on a [Custom Authorization Server](https://developer.okta.com/docs/guides/customize-tokens-returned-from-okta/main/) and want Arcade to use that instead.
- **Scopes**: defaults to `openid profile email`. The Org Authorization Server grants all three by default. If your app uses a Custom Authorization Server with a restricted scope policy, make sure the policy allows whatever scopes you list here.
