...
export const CLERK_SCREENSHOT_HEIGHT = 849 / IMAGE_SCALE_FACTOR;
...
In the **Add OAuth application** dialog:

- **Name**: a label your team will recognize, for example `Arcade User Source`.
- **Scopes**: select `openid`, `profile`, and `email`. These are the scopes Arcade requests by default.
- **Redirect URIs**: add `https://cloud.arcade.dev/oauth2/intermediate_callback`.

Click **Add** to create the application.

### Save the Client Secret

Clerk shows the **Client Secret** in a one-time dialog after the application is created. Copy the value and store it somewhere secure. The full secret is not retrievable from the Dashboard afterward, so a lost value requires rotating to a new one.

### Note the Client ID

After closing the secret dialog, Clerk redirects you to the application's settings page. The **Client ID** is listed under **Application credentials** and remains visible from there.

</Steps>

## Copy these values to Arcade

The Issuer URL is your Clerk instance's **Frontend API URL**. Find it on the **Configure** tab under **Developers** → **API keys**. It takes one of these forms:

- Development instance: `https://<verb-noun-NN>.clerk.accounts.dev` (Clerk-generated subdomain).
- Production instance: `https://clerk.<your-app-domain>.com` (your domain through Clerk's DNS setup).

Copy these values into Arcade's User Source form:

| Arcade field | Where to find it in Clerk |
|---|---|
| **Issuer URL** | The Frontend API URL exactly as Clerk shows it on the **API keys** page. Don't add a trailing slash. Clerk emits the issuer without one and Arcade matches the value exactly. |
| **Client ID** | The **Client ID** under **Application credentials** on the OAuth application's settings page. |
...
- **Client secret rotation**: Clerk lets you rotate the Client Secret from the OAuth application's settings page in the Dashboard. See [Rotate the client secret](/guides/user-sources#rotate-the-client-secret) in the User Sources guide for the Arcade-side procedure.
