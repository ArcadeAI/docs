...
These three are the OIDC scopes Arcade requests by default. `profile` is what unlocks Entra's directory-wide user identifier, which you'll see again in the [Advanced section](#advanced-settings) below; without it Entra won't include that claim in the ID token even if you configure it everywhere else.
...
{/* TODO: replace screenshot — needs all three of openid, profile, email checked. */}
...
Leave Arcade's other defaults alone unless you have a specific reason to change them. See [Advanced settings](#advanced-settings) below.
...
You don't need to touch the **Advanced** section in the User Source form for a standard Entra ID setup, but two settings live there for cases where the defaults don't fit:

- **Subject Claim**: defaults to `sub`, which Entra issues as a stable, app-specific identifier. Switch to `oid` if you need the same user to resolve to the same identifier across multiple User Sources or app registrations in the same tenant. For example, when you want one Arcade user identity to follow a person across multiple gateway projects backed by different User Sources. The trade-off is privacy: `sub` is pairwise per app, so different apps can't correlate the same person, while `oid` is the user's directory-wide identifier. Whichever you pick, make sure the **profile** permission you granted above is in place. Entra only emits `oid` when `profile` is in the requested scopes.
- **Scopes**: defaults to `openid profile email`, which matches the API permissions you granted in the step above. Add a scope here only if you've configured a custom claim on the ID token that depends on it.

See [Advanced: scopes and subject claim](/guides/user-sources#advanced-scopes-and-subject-claim) in the User Sources overview for the general guidance.
...
- **Custom subject claims**: If neither `sub` nor `oid` fits, configure the value you want as an [optional claim](https://learn.microsoft.com/en-us/entra/identity-platform/optional-claims#configure-optional-claims-in-your-application) on Entra's ID token, then set the User Source's **Subject Claim** to its name. Arcade can read any claim that ends up in the ID token; it doesn't have access to access tokens or other token types. See [Advanced: scopes and subject claim](/guides/user-sources#advanced-scopes-and-subject-claim) in the User Sources overview for what makes a good claim value. In particular, avoid claims that aren't both stable and never-reused.
