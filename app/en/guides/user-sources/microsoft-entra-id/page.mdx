...
The default Subject Claim, `sub`, identifies users with a per-app pseudonymous identifier. That's the right pick for most setups. If you'd rather see the user's directory-wide identifier in Arcade logs and gateway requests, useful when auditing against your Entra directory or correlating activity across multiple User Sources, open the form's **Advanced** section and change **Subject Claim** to `oid`.

`oid` only appears in the ID token when `profile` is in the requested scopes. You granted the `profile` permission in the step above, and `profile` is in the default scope set, so nothing else changes.
