...
{/* Exempted from glossary auto-link via lib/remark-glossary.ts; see User Sources overview for the Arcade-specific glossary terms. */}
...
{/* TODO: screenshot—Supported account types dropdown + Redirect URI fields */}
...
- **Single-tenant recommendation**: If you're not sure which "Supported account types" option to pick, choose **Single tenant only**. You can change it later only by registering a new app, so the safe default is the narrower option.
