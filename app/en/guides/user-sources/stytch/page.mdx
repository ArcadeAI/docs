...
- **Subject Claim**: Stick with Arcade's default Subject Claim, `sub`, which is the Stytch user's stable identifier. Stytch's Connected Apps issues a fixed set of [standard OIDC claims](https://stytch.com/docs/api-reference/consumer/api/connected-apps/tokens/connected-app-id-token-object) on the ID token (`iss`, `sub`, `aud`, `exp`, `nbf`, `iat`, plus profile, email, and phone claims when those scopes apply). There's no custom-claim template for the ID token today, so the standard claims are the only available picks. See [Advanced: scopes and subject claim](/guides/user-sources#advanced-scopes-and-subject-claim) in the overview for what makes a good claim.
