...
Following the example above, our Slack MCP Server offers the [`Slack.SendMessage`](/resources/integrations/social/slack#slacksendmessage) tool, which accepts a `username` as argument, matching exactly both the action and argument value expected to be present in the LLM context window.
