...
Learn how to build MCP tools that securely access secrets like API keys and passwords through Arcade's context system.
...
Secrets enable you to securely deploy a function that requires sensitive information at runtime. And while these can consume directly from the runtime environment inside your tool function, this becomes inconvenient, expensive, hard to maintain, and insecure when deploying at scale.

For example, if your tool requires an API key to use an external service, but only _after_ doing some computationally expensive work, you need to ensure that the API key is present _before_ the computationally expensive work occurs. The function below would fail if the API key is not present.
...
When the tool executes, it will return: `"Got SECRET_KEY of length..."`. In a real world application, you would use this secret to connect to a remote database, API, etc.
...
- **Handle missing secrets:** Use try/except when tools access secrets
...
- **Secure Access:** Tools access secrets through context, not imported directly
