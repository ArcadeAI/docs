...
This guide shows you how to build a Mastra agent and workflow that integrates Gmail and Slack using Arcade's MCP tools.
...
- `executeOrAuthorizeZodTool`: Handles tool execution and returns authorization URLs when users authorize
  ...
  execute: async ({ inputData }) => {
    ...
    try {
      ...
      if (error.status === 403 || error.message?.includes("authorization")) {
        ...
        return {
          success: false,
          message: `Slack authorization requires additional access. Please visit: ${slackAuth.url}`,
...
3. If authorization requires additional access, the workflow returns an auth URL. Visit the URL, complete authorization, then run the workflow again.
...
- **Authorization is automatic**: The `executeOrAuthorizeZodTool` factory handles auth flows. When a tool requires authorization, it returns a URL for the user to visit.
- **Workflows require explicit auth handling**: Unlike agents, workflows don't have built-in auth handling. Catch 403 errors, call `arcade.auth.start()`, and pass the auth URL through your workflow steps.
...
<Callout type="info">
  **Building a multi-user app?** This tutorial uses a single `ARCADE_USER_ID` for simplicity. For production apps where each user demands their own OAuth tokens, see [Secure auth for production](/guides/user-facing-agents/secure-auth-production) to learn how to dynamically pass user IDs and handle per-user authorization.
</Callout>
