...
The rest of the code uses these variables to customize the agent and manage the tools. Feel free to configure them to your liking. Here, we use the `EventListener` class to suppress CrewAI's rich panel output, which is useful for debugging but very verbose for an interactive session like the one you're building.
