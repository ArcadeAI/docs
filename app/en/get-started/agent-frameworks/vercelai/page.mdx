...
This guide shows you how to build a browser-based chatbot that uses Arcade's Gmail and Slack tools.

You'll create a Next.js application that integrates the [Vercel AI SDK](https://sdk.vercel.ai/) with Arcade's tool ecosystem. Your users will be able to read emails, send messages, and interact with Slack through a conversational interface with built-in authentication. The Vercel AI SDK is a TypeScript toolkit for building AI-powered applications that provides streaming responses, framework-agnostic support for React, Next.js, Vue, and more, plus AI provider switching. This guide uses **Vercel AI SDK v6**.
