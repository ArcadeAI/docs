...
Learn how to integrate Arcade tools with LangChain agents using interrupts and checkpointers.

LangChain is a popular agentic framework that abstracts a lot of the complexity when building AI agents. LangGraph, a lower level orchestration framework that offers more control over the inner flow of the agent, forms the foundation of LangChain.
...
This includes several imports, break them down:

- Arcade imports:
  - `AsyncArcade`: The Arcade client, interacts with the Arcade API.
  - `ToolDefinition`: The tool definition type, defines the shape of the tools that the agent can use.
- LangChain imports:
  - `create_agent`: Creates a LangChain agent using the ReAct pattern.
  - `AIMessage`: The message type for the agent's response.
  - `ToolMessage`: The message type for the agent's tool calls.
  - `StructuredTool`: LangChain's tool definition type, defines the shape of the tools that the agent can use.
  - `RunnableConfig`: The configuration type for the agent's run, includes the thread ID and other configuration options.
  - `ChatOpenAI`: The LLM to use for the agent's response, specific to OpenAI models.
  - `MemorySaver`: Stores the agent's state, and checkpointing and interrupts require it.
...
- Other imports:
  ...
  - `os`: The operating system module, interacts with the operating system.
  - `typing` imports: Type hints provide type safety.
  - `pydantic` imports: Data validation and model creation when converting Arcade tools to LangChain tools.
...
# The Arcade User ID identifies who authorizes each service.
ARCADE_USER_ID = os.getenv("ARCADE_USER_ID")
# This determines which MCP server provides the tools, you can customize this to make a Notion agent. All tools from the MCP servers defined in the array will be in use.
MCP_SERVERS = ["Slack"]
...
# This determines which LLM the agent will use
MODEL = "gpt-5-nano"
```

### Write a helper function to convert Arcade tools to LangChain tools

Here you convert the Arcade tools to LangChain tools. You use the `arcade_schema_to_pydantic` function to convert the Arcade tool definition to a Pydantic model, and then use the model to define a `StructuredTool` and create a LangChain tool.
...
def arcade_schema_to_pydantic(tool_def: ToolDefinition) -> type[BaseModel]:
    try:
        ...
        for param in tool_def.input.parameters or []:
            ...
            param_description = param.description or "The system provides no description."
...
async def arcade_to_langchain(
    ...
    async def tool_function(config: RunnableConfig, **kwargs: Any) -> Any:
        ...
        if not user_id:
            raise ValueError("You must provide a User ID to execute Arcade tools")
        ...
        if auth_response.status != "completed":
            ...
            if not authorized:
                raise RuntimeError(
                    f"You did not complete authorization for tool {arcade_tool.name}"
                )
...
async def get_arcade_tools(
    ...
    # if no tools or MCP servers exist, raise an error
    if not tools and not mcp_servers:
        raise ValueError(
            "No tools or MCP servers exist to retrieve tool definitions")
