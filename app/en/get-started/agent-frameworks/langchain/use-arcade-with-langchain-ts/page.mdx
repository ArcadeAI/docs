...
Learn how to integrate Arcade tools into LangChain agents using TypeScript.

LangChain is a popular agentic framework that abstracts a lot of the complexity of building AI agents. LangChain builds on top of LangGraph, a lower level orchestration framework that offers more control over the inner flow of the agent.
...
This is a number of imports, break them down:

- Arcade imports:
  - `Arcade`: This is the Arcade client, which allows interaction with the Arcade API.
...
- LangChain imports:
  ...
  - `MemorySaver`: Stores the agent's state, and the application requires it for checkpointing and interrupts.
- Other imports:
...
The application uses these variables in the rest of the code to customize the agent and manage the tools. Feel free to configure them to your liking.
...
// This determines which LLM LangChain uses inside the agent
...
This is a wrapper around the `executeZodTool` function. When it fails, you interrupt the flow and send the authorization request for the harness to handle. If the user authorizes the tool, the harness will reply with an `{authorized: true}` object, and LangChain will retry the tool call without interrupting the flow.
...
In LangChain, you must "resolve" each interrupt for the flow to continue. In response to an interrupt, you need to return a decision object with the information needed to resolve the interrupt. In this case, the decision is whether the authorization was successful, in which case LangChain retries the tool call, or if the authorization failed, the flow will interrupt with an error, and the agent will decide what to do next.
...
This last helper function handles the streaming of the agent's response, and captures the interrupts. When the application detects an interrupt, it adds the interrupt to the `interrupts` array, and interrupts the flow. If there are no interrupts, it will just stream the agent's response to your console.
...
rl.pause();
    ...
    try {
      ...
      while (true) {
        ...
        if (interrupts.length === 0) {
          break; // No more interrupts, the application is done
        }
