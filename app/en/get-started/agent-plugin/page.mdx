...
The Arcade agent plugin bundles a connection to Arcade's hosted MCP gateway with two skills, three commands, and the `arcade-operator` subagent. The gateway gives your agent thousands of tools for email, calendar, chat, issues, docs, and dozens of other apps. The rest of the plugin helps your agent choose and use those tools. You authorize each app in the browser the first time it comes up, with no API keys to create and no gateway to configure.
...
For [Claude Desktop](/get-started/agent-plugin/tools-only/claude-desktop), add the plugin from a marketplace instead of running `npx`. Using another client that does not load plugins? Connect to the same gateway with the [tools-only Arcade bundle](/get-started/agent-plugin/tools-only).
...
Reports whether the gateway is reachable, whether you have signed in, and which apps you have authorized.

#### `/arcade-connect <app>`

Authorizes an app on demand, so you are not interrupted mid-task.

#### `/arcade-apps`

Lists the apps you have connected and lets you disconnect one.
...
The `arcade-operator` subagent runs an app task in its own context and reports back just the result. Arcade exposes a lot of tools, and searching them fills a conversation with noise that pushes out your actual work. The operator absorbs that.
...
This gateway includes every [optimized toolkit](/resources/integrations) in the catalog. Try asking your agent:
