...
Using Claude Desktop, or a client that does not load plugins? Those connect to the same gateway as a [tools-only MCP server](/get-started/agent-plugin/tools-only).
...
For any other client, add the gateway URL by hand. See [Tools-only MCP server](/get-started/agent-plugin/tools-only).
