...
export const AUTH_OPTIONS_WIDTH = 740;
export const AUTH_OPTIONS_HEIGHT = 1024;
...
Those defaults match Arcade Auth and [User Sources](/operate/identity/user-sources). Do not switch to **Sign in when needed** or **No sign-in**. Those options skip or delay the OAuth flow Arcade uses. Do not switch to **Register automatically** or **Use your own OAuth client**. Arcade already supports Claude's published identity (CIMD).

Then click **Add**.

<Image
  alt={"Claude Add custom connector dialog with Sign in now and Use Claude's published identity marked Detected"}
  className="max-w-full mt-4"
  src={"/images/mcp-gateway/claude-desktop/step-3-auth-options.png"}
