...
Arcade tools are designed to be used with LLMs. This requires you to implement a harness that orchestrates the interaction between the LLM, the tools, and the user. While typically this is abstracted away by agentic frameworks, this guide will show you how to implement a simple harness from scratch, so that the underlying mechanics are clear.
