...
import { EarlyAccessRegistrySurvey } from "@/app/components/early-access-registry-survey";

# The Arcade Registry

The **Arcade Registry** is how agentic tool developers share their work with the world.  Arcade.dev is collecting all the integrations that agents will ever need in one place -  think HuggingFace or Pypi but for LLM tools.  A powerful community of open source and for-profit developers building out robust and evaluated workflows is how agents can be elevated to being *truly* useful.

<button
  onClick={() => {
    document.getElementById('early-access-registry-survey-header')?.scrollIntoView({ behavior: 'smooth' });
  }}
  className="mb-6 inline-flex items-center justify-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary/90"
>
  Take the survey
</button>
