...
If you are using the stdio transport, `arcade configure` will assume the entrypoint to your MCP server is `server.py` and will set the working directory to the root of your project. You can override this with the `--entrypoint` (or `-e`) option:
