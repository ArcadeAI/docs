...
<Callout type="warning">
  Store all credentials securely! Treat them like passwords. You cannot retrieve
  them again.
</Callout>
...
1. Make the app public: **Advanced** → **Make public**
2. Add the app to the organization: **Install app** → Select your organization
...
**Solution**: Check app settings → verify all required permissions are approved.
...
**Solution**: Settings → Applications → Installed GitHub Apps → Click app → Configure → Add repositories
...
- Enable in app settings → User permissions
- These actions act as the user, not as the app
...
_💡 Remember: Always use GitHub Apps (not OAuth Apps), store credentials securely, and rotate them regularly!_
