...
Running the CLI command `arcade new` will automatically create an example `worker.toml` file for the created MCP Server.
...
# You can specify a list of directories that contain a MCP Server.
# The directory must contain a pyproject.toml file or a setup.py file.
[worker.local_source]
packages = [ "./my_mcp_server1", "./my_mcp_server2"]
