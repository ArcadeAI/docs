...
You can secure your MCP server's `/mcp` endpoints with OAuth 2.1 Resource Server authentication. This turns your MCP server into a protected resource, enabling you to use tools with authorization and secrets over HTTP.
...
Resource Server authentication works alongside tool-level authorization. Resource Server auth secures access to your MCP server itself, while tool-level auth enables your tools to access third-party APIs on behalf of the authenticated user.
