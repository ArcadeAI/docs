...
<Tabs items={["macOS (Homebrew)", "Ubuntu/Debian (APT)", "Manual Download"]}>
  <Tabs.Tab>
    ```bash $HOMEBREW_REPOSITORY/etc/arcade-engine/engine.yaml ```
  </Tabs.Tab>
  <Tabs.Tab>```bash /etc/arcade-ai/engine.yaml ```</Tabs.Tab>
  <Tabs.Tab>
    ```bash $HOME/.arcade/engine.yaml ```
    <Callout type="info">
      To manually download the engine.yaml, you can get an example from the
      [Configuration
      Templates](/home/hosting-options/local-deployment/configure/templates#engineyaml)
...
<Tabs items={['macOS (Homebrew)', 'Ubuntu/Debian (APT)', 'Manual Download']}>
  ...
  ```
  <Callout type="info">To manually download the `engine.env`, refer to the [Configuration Templates](/home/hosting-options/local-deployment/configure/templa</Callout>ineenv).</Callout>
  </Tabs.Tab>
</Tabs>
