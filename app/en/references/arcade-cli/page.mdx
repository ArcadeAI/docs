...
<Callout type="info">
  Using Windows and PowerShell? Follow the [Windows environment setup](/en/get-started/setup/windows-environment) guide for install options with and without `uv`.
</Callout>
...
<Callout type="info">
If you previously had the `arcade-ai` package installed, you should uninstall it first. The old `arcade-ai` CLI has been replaced by `arcade-mcp`. See the [migration guide](/en/guides/create-tools/migrate-toolkits) for details on migrating from legacy toolkits.
</Callout>
