...
_Here's what's new at Arcade._
...
<Callout type="info">
  As of December 1, 2025, the MCP servers deployed via `arcade
  deploy` have been migrated to managed infrastructure. Please ensure you have the latest
...
- `[bugfix - 🐛]` Only serve worker endpoints if `ARCADE_WORKER_SECRET` environment variable sets
...
<Callout type="important">
  ...
  information. [Let others know what you think](/resources/contact-us).
</Callout>
...
- `[documentation - 📝]` `llms.txt` is now kept up to date and simplified. A new section was also added to the docs for [agentic development](/get-started/setup/connect-arcade-docs).
...
<Callout type="info">
  ...
  have a unique callback URL. This is a minor security change, but requires
  updating your OAuth configuration. This can be updated from the
  dashboard.
</Callout>
