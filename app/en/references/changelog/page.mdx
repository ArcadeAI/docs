...
Track updates, new features, and improvements to Arcade's platform, toolkits, and documentation.
...
<Callout type="important">
  Arcade has dramatically revamped the documentation to focus on making it easier to get started with Arcade. Update your links and let the team know what you think [here](/resources/contact-us).
</Callout>
...
<Callout type="info">
As of December 1, 2025, Arcade has migrated the MCP servers deployed via `arcade deploy` to the team's own managed infrastructure. Please ensure you have the latest version of the arcade CLI installed and that you are using the latest version of the `arcade-mcp-server` package.
</Callout>
...
**Platform and Engine**
- `[feature - 🚀]` Optional customization of OAuth request header format for upstream MCP servers
...
<Callout type="important">
  Arcade has updated the documentation to be more clear, consistent, and easier to navigate. This includes updated quickstarts, guides, and reference information. [Let the team know what you think](/resources/contact-us).
</Callout>

<Callout type="info">
  This week Arcade released `arcade-mcp`, the best way to build MCP servers. `arcade-mcp` supersedes the Arcade TDK. Learn more about it [here](/get-started/quickstarts/mcp-server-quickstart). Detailed reference information for `arcade-mcp` is available [here](/references/mcp/python).
</Callout>
...
- `[documentation - 📝]` `llms.txt` is now kept up to date and simplified. Arcade has also added a new section to the docs for [agentic development](/get-started/setup/connect-arcade-docs).
...
<Callout type="info">
  ...
  This enables Arcade to iterate more quickly and provide a better experience for
...
<Callout type="info">
  Version 2.0.0 of the Arcade Engine was released this week. Upgrading to version 2.0.0 is the recommended update for all self-hosted developers, and includes an important security fix for [secure OAuth flows](/guides/user-facing-agents/secure-auth-production). After upgrading, all projects will default to using the Arcade user verifier. If desired, you can then implement a custom user verifier in your application/agent and make the switch via the Arcade Dashboard.

Self-hosted Arcade developers cannot be deprecated into the old (insecure) behavior of skipping user verification once the Engine upgrades to version 2.0.0 or higher.
</Callout>
...
- `[feature - 🚀]` The engine will update log entries to reflect the tools that have been executed.
- `[feature - 🚀]` The engine will update documents to reflect changes to the secrets that have been deprecated.
- `[bugfix - 🐛]` The team released v9.6.1 of the engine which improves the upgrade experience.
- `[maintenance - 🔧]` The configuration sets value to 100 for the connection pool to improve performance.

**Misc**

- `[documentation - 📝]` [Arcade Examples](https://github.com/ArcadeAI/examples) can be updated to take advantage of the latest Arcade features.