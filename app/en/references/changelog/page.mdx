Looking at this changelog file against the style guide, I can identify several issues that need to be corrected:

# Changelog

Recent updates and improvements to Arcade's platform, MCP servers, and toolkits.

This page tracks all changes to Arcade's platform, toolkits, documentation, and related services. You'll find feature releases, bug fixes, and maintenance updates organized by date to help you stay current with the latest capabilities and improvements. Multiple issues required correction:

1. **Voice and Tone - "We" usage**: The style guide states to use "we" for Arcade, not for referring to the reader as part of the Arcade team. Arcade dramatically revamped its documentation to focus on making it easier to get started with Arcade. Update your links and let the Arcade team know what you think [here](/resources/contact-us)!

2. **Product names capitalization**: "Arcade.dev" should be "Arcade" according to the terminology section. Multiple issues need correction:

3. **Structure**: The page lacks the required intro line and first paragraph structure specified in the style guide.
...
This page tracks all changes to Arcade's platform, toolkits, documentation, and related services. You'll find feature releases, bug fixes, and maintenance updates organized by date to help you stay current with the latest capabilities and improvements.
...
<Callout type="important">
  Arcade has revamped its documentation to focus on making it easier to get started with Arcade.  Update your links and let Arcade know what you think [here](/resources/contact-us).
</Callout>

<Callout type="info">
  MCP Gateways now support OAuth!  Learn more about it [here](/guides/mcp-gateways).
</Callout>
...
<Callout type="info">
As of December 1, 2025, Arcade has migrated the MCP servers deployed via `arcade deploy` to its own managed infrastructure.  Please ensure you have the latest version of the arcade CLI installed and that you are using the latest version of the `arcade-mcp-server` package.
</Callout>
...
- `[bugfix - 🐛]` Only serve worker endpoints if `ARCADE_WORKER_SECRET` environment variable has a value
...
- `[maintenance - 🔧]` Allows the system to update organization and project names
...
<Callout type="important">
  Arcade has updated its documentation to be more clear, consistent, and easier to navigate.  This includes updated quickstarts, guides, and reference information. [Let the team know what you think](/resources/contact-us).
</Callout>


<Callout type="info">
  This week Arcade released `arcade-mcp`, the best way to build MCP Servers. `arcade-mcp` supersedes the Arcade TDK. Learn more about it [here](/get-started/quickstarts/mcp-server-quickstart).  Detailed reference information for `arcade-mcp` can be found [here](/references/mcp/python).
</Callout>

<Callout type="info">
  This week MCP Gateways become generally available. MCP Gateways allow you to federate the tools from multiple MCP Servers into a single collection for easier management, control, and access. Learn more about them [here](/guides/create-tools/mcp-gateways).
</Callout>

<Callout type="info">
  This week projects become generally available. Projects are a new way to organize your MCP Servers, tools, and secrets for easier management, control, and access.
</Callout>


**Toolkits**

- `[feature - 🚀]` `arcade-mcp` becomes generally available. Learn more about it [here](/get-started/quickstarts/mcp-server-quickstart).
...
- `[feature - 🚀]` MCP Gateways become generally available. Learn more about them [here](/guides/create-tools/mcp-gateways).
- `[feature - 🚀]` Projects become generally available.
...
- `[documentation - 📝]` `llms.txt` remains up to date and simplified.  Arcade has also added a new section to the docs for [agentic development](/get-started/setup/connect-arcade-docs).
...
<Callout type="info">
  This week Arcade released a new pricing model for Arcade which will be better for
...
<Callout type="info">
  This week Arcade enforced a new requirement for all OAuth providers: they must
  have a unique callback URL. This is a minor security change, but does require
  you to update your OAuth configuration. This can update from the
  dashboard.
</Callout>
...
- `[feature - 🚀]` Self-service plan selection for Arcade Cloud and payment becomes available.
...
<Callout type="info">
  The team released Version 2.0.0 of the Arcade Engine this week.  Upgrading to version 2.0.0 gets the recommendation for all self-hosted developers, and includes an important security fix for [secure OAuth flows](/guides/user-facing-agents/secure-auth-production).  After upgrading, all projects will default to using the Arcade user verifier.  If desired, you can then implement a custom user verifier in your application/agent and make the switch via the Arcade Dashboard.

Self-hosed Arcade developers cannot receive deprecated support for the old (insecure) behavior of skipping user verification once the Engine upgrades to version 2.0.0 or higher.
...
- `[documentation - 📝]` replaced creating MCP Server video