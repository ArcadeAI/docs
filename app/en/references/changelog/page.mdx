...
Track updates to Arcade platform features, MCP servers, and documentation.
...
<Callout type="important">
  Documentation has been dramatically revamped to focus on making it easier to
...
<Callout type="info">
  This week the team released `arcade-mcp`, the best way to build MCP servers.
...
Self-hosed Arcade developers cannot be deprecated into the old (insecure) behavior of skipping user verification once the Engine is upgraded to version 2.0.0 or higher.
...
- `[documentation - 📝]` replaced creating MCP server video with full tutorial