---
title: "Middleware - Arcade MCP Python reference"
...
Base class for MCP middleware with typed handlers for each MCP method. Subclass this to create custom middleware.

Each handler receives the context and a `call_next` function. Call `await call_next(context)` to continue the chain, or return early to short-circuit.
...
Handle `tools/call` requests. Override to add tool-specific processing.
...
Handle `tools/list` requests. Override to filter or modify the tool list.
...
Create a copy of the context with updated fields.
...
|------|------|-------------|
| `*middleware` | `Middleware` | One or more middleware instances (variadic positional arguments) |
...
Logs all MCP messages with timing information. Inherits from `Middleware`.
...
|-----------|------|-------------|---------|
| `log_level` | `str` | The log level to use for message logging | `'INFO'` |
...
| Parameter | Type | Description | Default |
|-----------|------|-------------|---------|
| `mask_error_details` | `bool` | When `True`, returns generic error messages instead of detailed ones (recommended for production) | `True` |

Overrides:
- `on_message` -- catches exceptions and converts them to `JSONRPCError` responses
- `on_call_tool` -- catches exceptions during tool execution and returns them as `CallToolResult` with `isError=True`
