...
<Callout type="warning">
  Types marked with 📘 are not yet documented in the Python SDK reference (though
...
| You want to... | Use |
|----------------|-----|
| Access validated input | `input` |
| Get OAuth token | `authorization.token` |
| Read a secret | `getSecret('KEY')` |
| Log to AI client | `log.info()`, `log.error()` |
| Report progress | `progress.report(current, total)` |
| Read a resource | `resources.get(uri)` |
| Call another tool | `tools.call(name, args)` |
| Ask user for input | `ui.elicit(message, schema)` |
...
<Callout type="warning">
  **Important:** When `isError: true`, the AI client knows the tool failed and can decide whether to retry. Always set this flag when returning error information — don't just return an error message as regular content.
</Callout>
...
For most error cases, throw specific error types instead — see [Errors](/references/mcp/typescript/errors).
