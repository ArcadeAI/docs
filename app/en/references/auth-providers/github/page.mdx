...
- `count_stargazers`, `get_repository`, `list_repository_activities`, `get_file_contents`
- Required: Contents (Read), Metadata (Read)

**Restricted stargazer listing:**

- `list_stargazers`
- The app needs read access to repository metadata, and the authenticated user must be an administrator or collaborator on the repository
- Use `count_stargazers` when you only need the public aggregate count
...
### Stargazer listings require repository access

GitHub limits its list stargazers endpoint to repository administrators and
collaborators. `list_stargazers` can return `404 Not Found` or `403 Forbidden`
for a public repository even when the app has Metadata (Read) permission and
can otherwise read that repository.

Use `count_stargazers` when you only need the public number of stars. It reads
the repository's aggregate `stargazers_count` instead of enumerating users.

[Learn about GitHub's stargazer access restrictions](https://github.blog/changelog/2026-06-30-upcoming-access-restrictions-to-public-api-endpoints-and-ui-views/)
...
| **Can't list stargazers on a public repo**    | User isn't a repository administrator or collaborator | Use `count_stargazers` for the public count       |
