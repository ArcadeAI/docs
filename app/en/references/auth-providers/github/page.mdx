...
<Callout>
  ...
  [Arcade GitHub MCP Server](/resources/integrations/development/github) is what you want!
</Callout>
...
- The [Arcade GitHub MCP Server](/resources/integrations/development/github), which provides pre-built tools for interacting with GitHub
...
Arcade's security team selected **GitHub Apps** over OAuth Apps for the GitHub MCP server based on three critical factors:
...
<tbody>
<tr>
<td width="25%"><strong>🔐 Permission Model</strong></td>
<td>
...
- **Redirect URI**: Note the URL generated by Arcade - add this to your GitHub app's "Callback URL"
...
2. Click **Connections** → **Connected apps** → **Add OAuth Provider**
3. Select **Custom Provider** tab (NOT "Included Providers")
...
- **Copy the Redirect URI**: You'll need this for your GHES GitHub App callback URL
...
- **Token Settings**: Set **Response Content Type** to **application/x-www-form-urlencoded** (change from default "application/json"). GitHub's token endpoint returns form-encoded data unless the request asks for JSON.

#### Refresh Token Settings

- **Refresh Token Endpoint**: Set to the **same URL** as Token Endpoint
  - Example: `https://{your-ghes-host}/login/oauth/access_token`
- **Response Content Type**: **application/x-www-form-urlencoded** (change from default "application/json")
...
- **Authentication Method**: **Bearer Access Token** (change from default "Client Secret Basic")
...
the number of stargazers of the ArcadeAI/arcade-mcp repository.

The Arcade GitHub MCP Server has a tool for that, which simplifies the process.
...
Use the pre-built [Arcade GitHub MCP Server](/resources/integrations/development/github) for quick GitHub integration.
...
- `update_pull_request`, `create_pull_request`, `merge_pull_request`, `submit_pull_request_review`,
  `manage_pull_request_reviewers`, `create_reply_for_review_comment`, `create_review_comment`, `resolve_review_thread`, `manage_labels` (for pull requests)
...
- `list_projects`, `list_project_items`, `search_project_item`, `list_project_fields`
...
**Repository Tools** (14 tools):

**Basic (Read-only):**

- `count_stargazers`, `get_repository`, `list_repository_activities`, `list_stargazers`, `get_file_contents`, `search_code`
- Required: Contents (Read), Metadata (Read)

**Write Operations:**

- `create_branch`, `create_file`, `update_file_lines`
...
- [ ] Set Response Content Type to `application/x-www-form-urlencoded` for both the token and refresh token requests
- [ ] Enabled Token Introspection with correct settings:
  - [ ] Introspection endpoint: `https://{your-ghes-host}/api/v3/user`
  - [ ] Authentication Method: Bearer Access Token
  - [ ] Request Content Type: application/json
  - [ ] Triggers: both On Token Grant and On Token Refresh enabled
...
- [Arcade GitHub MCP Server](/resources/integrations/development/github)
