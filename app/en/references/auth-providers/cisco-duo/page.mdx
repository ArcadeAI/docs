...
- Note the **Redirect URI** generated by Arcade. You'll add it to your Duo application in a later step.
...
- Under **PKCE Settings**, turn on **Enable PKCE** and select **S256** as the **Code Challenge Method**. Duo is OAuth 2.1 and requires PKCE.
...
- Under **Refresh Token Settings**, set the **Refresh Token Endpoint** to `<issuer>/token`. The form requires it. Arcade only refreshes tokens when Duo issues a refresh token, which requires the **Refresh Token** grant type.
...
Copy the **Redirect URI** that Arcade generated and add it to your Duo application's **Sign-In Redirect URLs** (on the application's **General** tab), then save the Duo application.
...
- Deploying a [custom MCP server](/build/create-tools/tool-basics/build-mcp-server) to Arcade with [`arcade deploy`](/build/arcade-deploy), or
- [Adding an existing remote MCP server](/operate/governance/remote-mcp-servers) to your Arcade project.
...
Create an [MCP gateway](/operate/governance/mcp-gateways) that exposes your server's tools, then connect your client to the gateway URL (`https://api.arcade.dev/mcp/<your-gateway>`).
...
Tools that call the APIs, databases, and enterprise systems protected by Cisco Duo are built like any other Arcade tool, so follow the [Add user authorization to your tools](/build/create-tools/tool-basics/create-tool-auth) and [Build an MCP server](/build/create-tools/tool-basics/build-mcp-server) guides.
