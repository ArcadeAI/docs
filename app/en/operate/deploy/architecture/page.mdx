...
{/*
  ...
  canvas in both themes rather than inverting. In dark mode the image is
  dimmed to 83% and the panel behind it is set to neutral-300, the same value,
  so the two match seamlessly and the figure reads as a lit panel instead of a
  glaring rectangle. Brightness scales every channel equally, so the diagram's
  own contrast is unchanged. The figure stays inside the content column.
*/}
