...
- **"Agent cannot register as an OAuth client" during connection**: AWS Sign-In only permits Dynamic Client Registration from approved redirect URIs, and Arcade's is approved. This isn't something you fix through Arcade configuration; it's resolved on AWS's side.
