...
<Callout type="info">
  ...
  also ships a [ServiceNow toolkit](/resources/integrations/customer-support/servicenow)
  with pre-built tools for incidents, changes, catalog requests, knowledge
  articles, and the CMDB. That toolkit calls ServiceNow's REST APIs directly,
  so it needs neither MCP Server Console nor a Now Assist entitlement. Setting
  it up is covered in the
  [ServiceNow auth provider reference](/references/auth-providers/servicenow).

  Use MCP Server Console for what no generic toolkit could pre-build, since a
  customer defines these for their own instance:
  ...
  Need something neither one exposes? Build a custom
