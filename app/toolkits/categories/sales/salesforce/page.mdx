...
<Note>
  ...
  Salesforce OAuth 2.0 credentials as documented in [Salesforce Auth Provider](/home/hosting-options/auth-providers/salesforce).
</Note>
...
<Note>
  In order to use the Salesforce toolkit, you must [self-host the Arcade Engine](/home/hosting-options/local-deployment/install/local) and [configure the Salesforce auth provider](/home/hosting-options/auth-providers/salesforce). The Arcade Engine is available at `http://localhost:9099` by default. In the code examples below, if necessary, adjust the `base_url` (in Python) or `baseURL` (in JavaScript) parameter in the `Arcade` client constructor to match your environment.
</Note>
