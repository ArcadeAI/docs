...
<Tip>
  ...
  own tools](/home/authoring-tools/build-tools/create-a-toolkit) with the [X auth
  provider](/home/hosting-options/auth-providers/x#using-x-auth-in-custom-tools).
</Tip>
...
The Arcade X (Twitter) toolkit uses the [X auth provider](/home/hosting-options/auth-providers/x) to connect to users' X (formerly Twitter) accounts.
...
With a self-hosted installation of Arcade, you need to [configure the X auth provider](/home/hosting-options/auth-providers/x#configuring-x-auth) with your own X (formerly Twitter) app credentials.
